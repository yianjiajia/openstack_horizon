...
from openstack_dashboard.dashboards.cdn.cdn_domain_manager\
...
from openstack_dashboard.dashboards.cdn.cdn_domain_manager.models import CdnBillMethod

class IndexView(tables.DataTableView):
    # cdn domain list view
    table_class = domain_tables.DomainManagerTable
    template_name = 'cdn/cdn_domain_manager/index.html'

    def get_context_data(self, **kwargs):
        context = super(IndexView, self).get_context_data(**kwargs)
        context['current_mode'] = _(self.current_billing_mode)
        context['update_mode'] = _(self.update_billing_mode)
    ...
    def _get_domain(self):
        try:
            ...
            account_mode = CdnBillMethod.objects.filter(tenant_id=tenant_id)
            if not account_mode:
                bill_method = CdnBillMethod(tenant_id=tenant_id, current_type='cdnflow')
                bill_method.save()
                self.current_billing_mode = 'cdnflow'
                self.update_billing_mode = ''
            else:
                self.current_billing_mode = account_mode[0].current_type
                self.update_billing_mode = account_mode[0].update_type if account_mode[0].update_type else ''
...
class CreateView(forms.ModalFormView):
    ...
    template_name = 'cdn/cdn_domain_manager/create.html'
    success_url = reverse_lazy("horizon:cdn:cdn_domain_manager:index")
    page_title = _("Create Domain Name")
    submit_label = _("create")
    submit_url = reverse_lazy("horizon:cdn:cdn_domain_manager:create")
...
class VerifyView(forms.ModalFormView):
    ...
    template_name = 'cdn/cdn_domain_manager/verify.html'
    success_url = reverse_lazy("horizon:cdn:cdn_domain_manager:index")
    page_title = _("Verify Domain")
    submit_url = "horizon:cdn:cdn_domain_manager:action"
...
class UpdateTabView(tables.MultiTableView):
    ...
    template_name = 'cdn/cdn_domain_manager/detail.html'
    failure_url = 'horizon:cdn:cdn_domain_manager:index'
    modify_url = 'horizon:cdn:cdn_domain_manager:modify'
...
class UpdateDomainFormView(forms.ModalFormView):
    ...
    template_name = 'cdn/cdn_domain_manager/create.html'
    submit_url = 'horizon:cdn:cdn_domain_manager:modify'
    success_url = 'horizon:cdn:cdn_domain_manager:update'
        ...
        if handled:
            ...
            if domain.domain_id is not None:
                ...
                if ret.getMsg() == 'success':
                    ...
                    # 插入操作日志
                    Logger(self.request).create(resource_type='CDN', action_name='Update Domain Name',
...
class AddAccessFormView(forms.ModalFormView):
    ...
    submit_label = _("create")
    submit_url = 'horizon:cdn:cdn_domain_manager:addaccess'
        ...
        if handled:
            ...
            if domain.status == 'Deployed':
                ...
                if ret.getMsg() == 'success':
                    ...
                    # 插入操作日志
                    Logger(self.request).create(resource_type='CDN', action_name='Create Access',
                                                       resource_name='CDN', config=_('Domain: %s') %domain.domain_name,
                                                       status='Success')
                else:
...
class ModifyAccessFormView(forms.ModalFormView):
    ...
    submit_label = _("update")
    submit_url = 'horizon:cdn:cdn_domain_manager:modifyaccess'
        ...
        if handled:
            ...
            if domain.status == 'Deployed':
                ...
                if ret.getMsg() == 'success':
                    ...
                    # 插入操作日志
                    Logger(self.request).create(resource_type='CDN', action_name='Update Access',
                                                       resource_name='CDN', config=_('Domain: %s') %domain.domain_name,
                                                       status='Success')
                else:
                    messages.error(self.request, ret.getMsg())
...
class AddCacheFormView(forms.ModalFormView):
    ...
    submit_label = _("create")
    submit_url = "horizon:cdn:cdn_domain_manager:addcache"
        ...
        if handled:
            ...
            if domain.status == 'Deployed':
                ...
                if ret.getMsg() == 'success':
                    ...
                    # 插入操作日志
                    Logger(self.request).create(resource_type='CDN', action_name='Create Cache',
                                                       resource_name='CDN', config=_('Domain: %s') %domain.domain_name,
                                                       status='Success')
                else:
                    messages.error(self.request, ret.getMsg())
...
class ModifyCacheFormView(forms.ModalFormView):
    ...
    submit_label = _("update")
    submit_url = "horizon:cdn:cdn_domain_manager:modifycache"
        ...
        if domain_list.status == 'unverified' or domain_list.status == 'failed' or domain_list.status == 'addfailed':
            if domain.verify_file_check(str(domain_name)):
                ...
                if status == 202:
                    ...
                    # 插入操作日志
                    Logger(request).create(resource_type='CDN', action_name='Verify Domain',
                                                       resource_name='CDN', config=_('Domain: %s') %domain_name,
                                                       status='Success')
                else:
                    ...
                    Logger(request).create(resource_type='CDN', action_name='Verify Domain',
                                                       ...
                                                       resource_name='CDN', config=_('Audit Error Log(%s): %s') %(domain_name,
                                                                                                                 msg),
                                                       status='Error')
            else:
...
class ModifyAccountMode(forms.ModalFormView):
    ''' modify account mode '''
    form_class = domain_forms.ModifyAccountModeForm
    form_id = "modify_account_mode"
    modal_header = _("Modify Account Mode")
    template_name = 'cdn/cdn_domain_manager/create.html'
    success_url = reverse_lazy("horizon:cdn:cdn_domain_manager:index")
    page_title = _("Modify Account Mode")
    submit_label = _("Modify")
    submit_url = reverse_lazy("horizon:cdn:cdn_domain_manager:modifyaccountmode")

    def get_context_data(self, **kwargs):
        context = super(ModifyAccountMode, self).get_context_data(**kwargs)
        context['message'] = _("Warning:your account mode you modify,"
                               "will effect at next month,"
                               "and this month still use current account mode")
        return context
