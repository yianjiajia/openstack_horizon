...
from openstack_dashboard.dashboards.cdn.cdn_log_manager import tables \
    as log_table
...
class LogUsage(middware.BaseUsage):
    ...
    def get_usage_list(self, start, end):
        try:
            ...
            domain_quset = [i for i in Domain.objects.filter(tenant_id=tenant_id) if i.domain_id is not None
                            and i.status != 'deleted' and i.created_at.replace(tzinfo=None) < end]
                ...
                if ret.getLogs() is None:
                    ...
                    for j in ret.getLogs():
                        log_class = middware.LogData(domain=i.domain_name, log_url=j.url,
                                                     size=j.fileSize, begin=str(j.dateFrom), end=str(j.dateTo))
...
class LogOverview(middware.MyUsageView):
    ...
    template_name = 'cdn/cdn_log_manager/usage.html'
