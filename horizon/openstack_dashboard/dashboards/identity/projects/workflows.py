...
class ProjectQuotaAction(workflows.Action):
    ...
    region_id = forms.ChoiceField(label=_("Primary Region"),required=True)
    metadata_items = forms.IntegerField(min_value=-1, initial=1024,
                                        label=_("Metadata Items"))
    cores = forms.IntegerField(min_value=2, initial=32, label=_("VCPUs"))
    instances = forms.IntegerField(min_value=-1, initial=8, label=_("Instances"))
    injected_files = forms.IntegerField(min_value=-1, initial=40, 
                                        label=_("Injected Files"))
    injected_file_content_bytes = forms.IntegerField(min_value=-1, initial=32768, 
                                                     label=ifcb_label)
    volumes = forms.IntegerField(min_value=-1, initial=10, label=_("Volumes"))
    snapshots = forms.IntegerField(min_value=-1, initial=10, label=_("Volume Snapshots"))
    volume_gigabytes = forms.IntegerField(
        min_value=-1, initial=640, label=_("Size of Volumes(GB)"))
    snapshot_gigabytes = forms.IntegerField(
        min_value=-1, initial=640, label=_("Size of Snapshots (GB)"))
    ram = forms.IntegerField(min_value=-1, initial=32768, label=_("RAM (MB)"))
    floating_ips = forms.IntegerField(min_value=-1, initial=4, label=_("Floating IPs"))
    fixed_ips = forms.IntegerField(min_value=-1, initial=8, label=_("Fixed IPs"))
    security_groups = forms.IntegerField(min_value=-1, initial=2,
                                         label=_("Security Groups"), widget=forms.HiddenInput)
    security_group_rules = forms.IntegerField(min_value=-1, initial=20,
                                              label=_("Security Group Rules"))

    # Neutron
    security_group = forms.IntegerField(min_value=-1, initial=2,
                                        label=_("Security Groups"))
    security_group_rule = forms.IntegerField(min_value=-1, initial=20,
                                             label=_("Security Group Rules"))
    floatingip = forms.IntegerField(min_value=-1, initial=2, label=_("Floating IPs"))
    network = forms.IntegerField(min_value=-1, initial=1, label=_("Networks"))
    port = forms.IntegerField(min_value=-1, initial=128, label=_("Ports"))
    router = forms.IntegerField(min_value=-1, initial=1, label=_("Routers"))
    subnet = forms.IntegerField(min_value=-1, initial=1, label=_("Subnets"))
    pool = forms.IntegerField(min_value=-1, initial=1, label=_("Loadbalancers"))
    bandwidth = forms.IntegerField(min_value=-1, initial=10, label=_("Total Bandwidth (Mbps)"),
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.fields['metadata_items'].widget = forms.HiddenInput()
        self.fields['injected_files'].widget = forms.HiddenInput()
        self.fields['injected_file_content_bytes'].widget = forms.HiddenInput()
        self.fields['floating_ips'].widget = forms.HiddenInput()
        self.fields['fixed_ips'].widget = forms.HiddenInput()
        self.fields['security_groups'].widget = forms.HiddenInput()
        self.fields['security_group_rules'].widget = forms.HiddenInput()
        self.fields['security_group'].widget = forms.HiddenInput()
        self.fields['security_group_rule'].widget = forms.HiddenInput()
        self.fields['port'].widget = forms.HiddenInput()
        if policy.check((("identity", "project_admin_required"),), self.request):
            self.fields['cores'].validators.append(MaxValueValidator(32))
            self.fields['instances'].validators.append(MaxValueValidator(32))
            self.fields['volumes'].validators.append(MaxValueValidator(64))
            self.fields['snapshots'].validators.append(MaxValueValidator(64))
            self.fields['volume_gigabytes'].validators.append(MaxValueValidator(7680))
            self.fields['snapshot_gigabytes'].validators.append(MaxValueValidator(7680))
            self.fields['ram'].validators.append(MaxValueValidator(262144))
            self.fields['floatingip'].validators.append(MaxValueValidator(32))
            self.fields['network'].validators.append(MaxValueValidator(1))
            self.fields['port'].validators.append(MaxValueValidator(128))
            self.fields['router'].validators.append(MaxValueValidator(1))
            self.fields['subnet'].validators.append(MaxValueValidator(1))
            self.fields['bandwidth'].validators.append(MaxValueValidator(100))
            self.fields['network'].widget = forms.HiddenInput()
            self.fields['port'].widget = forms.HiddenInput()
            self.fields['router'].widget = forms.HiddenInput()
            self.fields['subnet'].widget = forms.HiddenInput()
        ...
        try:
            ...
            if default_role is None:
                default = getattr(settings,
                                  "OPENSTACK_KEYSTONE_DEFAULT_SUPPORT_ROLE", None)
...
class UpdateProject(workflows.Workflow):
    ...
    def _remove_roles_from_user(self, request, project_id, user_id,
                                current_role_ids):
        default_role = api.keystone.get_default_support_role(request)
        for id_to_delete in current_role_ids:
            # now only remove support
            if id_to_delete == default_role.id:
