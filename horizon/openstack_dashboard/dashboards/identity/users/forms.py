...
class BaseUserForm(forms.SelfHandlingForm):
    def __init__(self, request, *args, **kwargs):
        ...
        projects=[]
        if policy.check((("identity", "identity:list_children_projects"),),self.request):
            parent_id=self.request.user.project_id
...
class CreateUserForm(PasswordMixin, BaseUserForm):
    ...
    role_id = forms.ChoiceField(label=_("Role"),
                                required=PROJECT_REQUIRED,
                                widget=forms.HiddenInput())
    region_id = forms.ChoiceField(label=_("Primary Region"),
                                required=True)
    cdnLimit = forms.BooleanField(label=_("CDN Limit"),
                                   required=False,
                                   initial=False)
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            if 'cdnLimit' in data and data['cdnLimit']:
                cdnLimit = 1
            else:
                cdnLimit = 0
            ...
            #billing center  add by ying.zhou
            gift_balance = 0
            try:
            ...
            data_toBilling={"account":{'account_id':'','username':data['name'],'cash_balance':0,'gift_balance':gift_balance,'type':'normal','credit_line':0,'status':'normal','user_id':new_user.id}}
            try:
...
class UpdateUserForm(BaseUserForm):
    ...
    def update_billing_accout(self, request, project_id, data):
        client = billing.RequestClient(request)
        account = client.get_account(project_id)
        params = {}
        params['account'] = {}
        params['account']['username'] = data['name']
    ...
    def handle(self, request, data):
        ...
        try:
            if "email" in data:
                data['email'] = data['email'] or None
            if "company" in data:
                data['company'] = data['company'] or None
            if "telephone" in data:
                data['telephone'] = data['telephone'] or None
            if "contact" in data:
                data['contact'] = data['contact'] or None
            if "cdnLimit" in data:
                if data['cdnLimit']:
                    data['cdnLimit'] = 1
                else:
            ...
            self.update_billing_accout(request, data['project'], data)
            messages.success(request,
