...
class CreateBackupForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Volume ID: %s Back Name: %s") %(data['volume_id'], data['name'])
            api.logger.Logger(request).create(resource_type='volume', action_name='Create Volume Backup',
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Create Volume Backup',
                                                       resource_name='Volume', config= _('Unable to create volume backup.'),
                                                       status='Error')
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            messages.success(request, message % {'backup_name': backup_name,
                                                 'volume_id': volume_id})
            # operation log
            config = _("Volume ID: %s Backup Name: %s Backup ID: %s") %(volume_id, backup_name, backup_id)
            api.logger.Logger(request).create(resource_type='volume', action_name='Restored Volume Backup',
                                                       resource_name='Volume', config=config,
                                                       status='Success')
            ...
            exceptions.handle(request, msg, redirect=redirect)
             ...
             # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Restored Volume Backup',
                                                       resource_name='Volume', config= _('Unable to restore backup.'),
                                                       status='Error')
