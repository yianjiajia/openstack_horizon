...
class CreateBackupForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = '\n'.join(['Volume ID: '+ data['volume_id'], 'Back Name: ' + data['name']])
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Create Volume Backup',
                                                       resource_name='Volume', config='Unable to create volume backup',
                                                       status='Error')
...
class RestoreBackupForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = '\n'.join(['Volume ID: '+ volume_id, 'Backup Name: ' + backup_name, 'Backup ID: ' + backup_id])
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Restored Volume Backup',
                                                       resource_name='Volume', config='Unable to restore backup',
                                                       status='Error')
