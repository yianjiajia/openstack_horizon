...
from openstack_dashboard.dashboards.instances.images import utils
from openstack_dashboard.dashboards.instances.instances import tables
...
class CreateForm(forms.SelfHandlingForm):
    name = forms.CharField(max_length=50, label=_("Volume Name"),
                           required=False)
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Volume Name: %s Volume Count: %s Volume Size: %sG Volume Type: %s") %(data.get('name',''),
                                                                                          str(data['quantity']),
                                                                                          str(data['size']),
                                                                                          data['type'])
            api.logger.Logger(request).create(resource_type=resource_type, action_name='Create Volume',
...
class AttachForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Attach Volume To Instance',
                                                       resource_name='Volume', config=_('Volume ID: %s') %data['volume_id'],
                                                       status='Success')
            return True
        except Exception:
            redirect = reverse("horizon:storage:volumes:index")
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Attach Volume To Instance',
                                                       resource_name='Volume', config='Unable to attach volume.',
                                                       status='Error')
...
class CreateSnapshotForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            config = _("Snapshot Name: %s Volume ID: %s") %(data['name'], data['volume_id'])
            api.logger.Logger(request).create(resource_type='volume', action_name='Create Snapshot',
                                                       resource_name='Volume', config=config,
                                                       status='Success')
            return snapshot
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Create Snapshot',
                                                       resource_name='Volume', config=_('Unable to create volume snapshot.'),
                                                       status='Error')
...
class CreateTransferForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Created volume Transfer',
                                                       resource_name='Volume', config=_('Volume ID: %s') %volume_id,
                                                       status='Success')
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Created volume Transfer',
                                                       resource_name='Volume', config='Unable to create volume transfer.',
                                                       status='Error')
...
class AcceptTransferForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Accepted Volume Transfer',
                                                       resource_name='Volume', config=_('Volume ID: %s') %data['transfer_id'],
                                                       status='Success')
            return transfer
        except Exception:
            redirect = reverse("horizon:storage:volumes:index")
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Accepted Volume Transfer',
                                                       resource_name='Volume', config= _('Unable to accept volume transfer.'),
                                                       status='Error')
...
class UpdateForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            api.logger.Logger(request).create(resource_type='volume', action_name='Update Volume',
                                                       resource_name='Volume', config=config,
                                                       status='Success')
        except Exception:
            redirect = reverse("horizon:storage:volumes:index")
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Update Volume',
                                                       resource_name='Volume', config=_('Unable to update volume.'),
                                                       status='Error')
        ...
        if make_bootable != self.initial['bootable']:
            try:
                ...
                # operation log
                api.logger.Logger(request).create(resource_type='volume', action_name='Update Volume',
                                                       resource_name='Volume', config=_('Bootable: %s') % str(data['bootable']),
                                                       status='Success')
            except Exception:
                redirect = reverse("horizon:storage:volumes:index")
                exceptions.handle(request,
                ...
                # operation log
                api.logger.Logger(request).create(resource_type='volume', action_name='Update Volume',
                                                       resource_name='Volume', config= _('Unable to set bootable flag on volume.'),
                                                       status='Error')
...
class UploadToImageForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Volume ID: %s Image Name: %s Disk Format: %s") %(volume_id, data['image_name'], data['disk_format'])
            api.logger.Logger(request).create(resource_type='volume', action_name='Upload Volume To Image',
                                                       resource_name='Volume', config=config,
                                                       status='Success')
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Upload Volume To Image',
                                                       resource_name='Volume', config=_('Unable to upload volume to image for volume: "%s"')
                                                                                      % data['name'],
                                                       status='Error')
...
class ExtendForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Volume ID: %s Volume Size: %s") %(volume_id, str(data['new_size']))
            api.logger.Logger(request).create(resource_type='volume', action_name='Extend Volume',
                                                       resource_name='Volume', config=config,
                                                       status='Success')
            return volume
        except Exception:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Extend Volume',
                                                       resource_name='Volume', config=_('Unable to extend volume.'),
                                                       status='Error')
...
class RetypeForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Volume ID: %s Volume Type: %s") %(volume_id, data['volume_type'])
            api.logger.Logger(request).create(resource_type='volume', action_name='Change Volume Type',
                                                       resource_name='Volume', config=config,
                                                       status='Success')
            return True
        except Exception:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='volume', action_name='Change Volume Type',
                                                       resource_name='Volume', config=_('Unable to change the volume type for volume: "%s"') % data['name'],
                                                       status='Error')
