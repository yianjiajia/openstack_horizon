...
DELETABLE_STATES = ("available", "error", "error_extending", "error_deleting")
...
class DeleteVolume(VolumePolicyTargetMixin, tables.DeleteAction):
    ...
    def delete(self, request, obj_id):
        ...
        # operation log
        api.logger.Logger(request).create(resource_type='volume', action_name='Remove Volume',
                                                       resource_name='Volume', config=_('Volume ID: %s') %obj_id,
                                                       status='Success')
...
class AcceptTransfer(tables.LinkAction):
    ...
    def allowed(self, request, datum):
        ...
        if account_is_frozen:
            ...
            self.classes = [c for c in self.classes if c != "disabled"]
        return super(AcceptTransfer, self).allowed(request, datum)
...
class VolumeSnapshotNameColumn(tables.Column):
    def get_raw_data(self, volume):
        snapshot_id = volume.snapshot_id
        if snapshot_id:
            snapshot_name = volume.snapshot_id
    ...
    def get_link_url(self, volume):
        snapshot_id = volume.snapshot_id
        if snapshot_id:
            return reverse(self.link, args=(snapshot_id,))
...
class VolumesTable(VolumesTableBase):
    BOOTABLE_DISPLAY_CHOICES = (
        (True, pgettext_lazy("Current bootable of a Volume",
                                    u"True")),
        (False, pgettext_lazy("Current bootable of a Volume", u"False")))

    name = tables.Column("name",
                         verbose_name=_("Name"),
                         link="horizon:storage:volumes:volumes:detail")
    snapshot = VolumeSnapshotNameColumn("snapshot",
                                verbose_name=_("Parent Snapshot"),
    ...
    bootable = tables.Column('is_bootable',
                             ...
                             display_choices=BOOTABLE_DISPLAY_CHOICES)
    '''
