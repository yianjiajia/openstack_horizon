...
from openstack_dashboard.dashboards.security.firewalls \
...
class IndexView(tabs.TabView):
    ...
    template_name = 'security/firewalls/details_tabs.html'
        ...
        if obj_type == 'rule':
            for obj_id in obj_ids:
                try:
                    ...
                    # operation log
                    config = _('Rule ID: %s') % obj_id
                    api.logger.Logger(request).create(resource_type='firewall', action_name='Delete Firewall Rule',
                    ...
                    # operation log
                    api.logger.Logger(request).create(resource_type='firewall', action_name='Delete Firewall Rule',
                                                       resource_name='Firewall', config=_('Unable to delete rule. %s') % e,
                                                       status='Error')
        if obj_type == 'policy':
            for obj_id in obj_ids:
                try:
                    ...
                    # operation log
                    config = _('Policy ID: %s') % obj_id
                    api.logger.Logger(request).create(resource_type='firewall', action_name='Delete Firewall Policy',
                                                       resource_name='Firewall', config=config,
                                                       status='Success')
                except Exception as e:
                    exceptions.handle(request,
                    ...
                    # operation log
                    api.logger.Logger(request).create(resource_type='firewall', action_name='Delete Firewall Policy',
                                                       resource_name='Firewall', config= _('Unable to delete policy. %s') % e,
                                                       status='Error')
        if obj_type == 'firewall':
            for obj_id in obj_ids:
                try:
                    ...
                    # operation log
                    config = _('Firewall ID: %s') % obj_id
                    api.logger.Logger(request).create(resource_type='firewall', action_name='Remove Firewall',
                                                       resource_name='Firewall', config=config,
                                                       status='Success')
                except Exception as e:
                    exceptions.handle(request,
                    ...
                    # operation log
                    api.logger.Logger(request).create(resource_type='firewall', action_name='Remove Firewall',
                                                       resource_name='Firewall', config=_('Unable to delete firewall. %s') % e,
                                                       status='Error')
...
class UpdateRuleView(forms.ModalFormView):
    ...
    template_name = "security/firewalls/updaterule.html"
    context_object_name = 'rule'
    modal_header = _("Edit Rule")
    submit_label = _("Save Changes")
    submit_url = "horizon:security:firewalls:updaterule"
    success_url = reverse_lazy("horizon:security:firewalls:index")
...
class UpdatePolicyView(forms.ModalFormView):
    ...
    template_name = "security/firewalls/updatepolicy.html"
    context_object_name = 'policy'
    modal_header = _("Edit Policy")
    submit_label = _("Save Changes")
    submit_url = "horizon:security:firewalls:updatepolicy"
    success_url = reverse_lazy("horizon:security:firewalls:index")
...
class UpdateFirewallView(forms.ModalFormView):
    ...
    template_name = "security/firewalls/updatefirewall.html"
    context_object_name = 'firewall'
    modal_header = _("Edit Firewall")
    submit_label = _("Save Changes")
    submit_url = "horizon:security:firewalls:updatefirewall"
    success_url = reverse_lazy("horizon:security:firewalls:index")
...
class InsertRuleToPolicyView(forms.ModalFormView):
    ...
    template_name = "security/firewalls/insert_rule_to_policy.html"
    context_object_name = 'policy'
    submit_url = "horizon:security:firewalls:insertrule"
    submit_label = _("Save Changes")
    success_url = reverse_lazy("horizon:security:firewalls:index")
...
class RemoveRuleFromPolicyView(forms.ModalFormView):
    ...
    template_name = "security/firewalls/remove_rule_from_policy.html"
    context_object_name = 'policy'
    submit_label = _("Save Changes")
    submit_url = "horizon:security:firewalls:removerule"
    success_url = reverse_lazy("horizon:security:firewalls:index")
...
class AddRouterToFirewallView(RouterCommonView):
    ...
    template_name = "security/firewalls/add_router_to_firewall.html"
    submit_url = "horizon:security:firewalls:addrouter"
...
class RemoveRouterFromFirewallView(RouterCommonView):
    ...
    template_name = "security/firewalls/remove_router_from_firewall.html"
    submit_url = "horizon:security:firewalls:removerouter"
