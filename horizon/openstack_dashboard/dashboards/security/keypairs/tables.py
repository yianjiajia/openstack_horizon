...
class DeleteKeyPairs(tables.DeleteAction):
    ...
    def delete(self, request, obj_id):
        ...
        # operation log
        config = _('Keypair ID: %s') %obj_id
        api.logger.Logger(request).create(resource_type='keypair', action_name='Delete Keypair',
...
class ImportKeyPair(tables.LinkAction):
    ...
    url = "horizon:security:keypairs:import"
    classes = ("ajax-modal",)
    icon = "upload"
...
class CreateKeyPair(tables.LinkAction):
    ...
    url = "horizon:security:keypairs:create"
    classes = ("ajax-modal",)
    icon = "plus"
    policy_rules = (("compute", "compute_extension:keypairs:create"),)
...
class KeypairsTable(tables.DataTable):
    detail_link = "horizon:security:keypairs:detail"
