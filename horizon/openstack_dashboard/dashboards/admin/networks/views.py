...
from openstack_dashboard.dashboards.network.networks import views as user_views
...
class IndexView(tables.DataTableView):
    ...
    def get_data(self):
        ...
        prev_marker = self.request.GET.get(
            networks_tables.NetworksTable._meta.prev_pagination_param)
        if prev_marker is not None:
            ...
            marker = self.request.GET.get(
                networks_tables.NetworksTable._meta.pagination_param)
        ...
        try:
            networks = api.neutron.network_list_admin(self.request, **search_opts)
        except Exception:
            networks = []
        ...
        if networks:
            ...
            if len(networks) > page_size:
                if page_reverse:
                    networks = networks[1:len(networks)]
                else:
                    networks.pop()
                self._more = True
