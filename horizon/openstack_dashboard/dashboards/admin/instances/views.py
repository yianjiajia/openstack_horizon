...
from openstack_dashboard.dashboards.instances.instances import views
...
class AdminIndexView(tables.DataTableView):
    ...
    def get_data(self):
        ...
        if prev_marker is not None:
            ...
            marker = self.request.GET.get(
                project_tables.AdminInstancesTable._meta.pagination_param, None)
        search_opts = self.get_filters({'marker': marker, 'paginate': True, 'sort_dir': sort_dir})
        ...
        if 'ip' in search_opts:
            floatingIp = api.network.floating_ip_list_by_ip(
                            self.request, 
                            floating_ip_address=search_opts.pop('ip'))
            if not floatingIp:
                return []
            else:
                if 1 != len(floatingIp):
                        msg = _("Multi Floating ip returned")
                        exceptions.handle(self.request, msg)
                else:
                    search_opts['uuid'] = floatingIp[0]['instance_id']

        try:
            instances, self._more, self._prev = api.nova.server_list_admin(
                self.request,
                search_opts=search_opts,
                all_tenants=True)
            if prev_marker is not None:
                instances = sorted(instances, key=attrgetter('created'),
                                reverse=True)
        except Exception:
            self._more = False
            self._prev = False
        ...
        if instances:
            '''
            #Don't update the addresses of instance, expect to display the floating ip in list
            try:
    ...
    def get_filters(self, filters):
        ...
        if filter_action.is_api_filter(filter_field):
            ...
            if filter_field and filter_string:
                if filter_field == 'instance_name':
                    filter_field = 'id'
                    try:
                        filter_string = str(int(filter_string.split('-')[1], 16))
                    except:
                        pass
