...
class ManageVolume(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            api.cinder.volume_manage(request,
                                 host=data['host'],
...
class UnmanageVolume(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            api.cinder.volume_unmanage(request, self.initial['volume_id'])
            messages.success(
                request,
...
class CreateVolumeType(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            volume_type = api.cinder.volume_type_create(request,
                                                    data['name'])
...
class UpdateStatus(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            api.cinder.volume_reset_state(request,
            ...
            # operation log
            config = '\n'.join(['Volume ID: '+ self.initial['volume_id']])

            api.logger.Logger(request).create(resource_type='volume', action_name='Update Volume Status',
            ...
            api.logger.Logger(request).create(resource_type='volume', action_name='Update Volume Status',
                                                       resource_name='Volume', config='Unable to update volume status',
                                                       status='Error')
...
class CreateQosSpec(forms.SelfHandlingForm):
    ...
    consumer = forms.ChoiceField(label=_("Consumer"),
                                 choices=api.cinder.CONSUMER_CHOICES)
