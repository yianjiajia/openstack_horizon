...
from horizon.utils.functions import set_billing_account_to_session

from openstack_dashboard import api
from openstack_dashboard.usage import quotas

from openstack_dashboard.dashboards.network.network_topology.instances \
    import tables as instances_tables
from openstack_dashboard.dashboards.network.network_topology.ports \
    import tables as ports_tables
from openstack_dashboard.dashboards.network.network_topology.routers \
    import tables as routers_tables

from openstack_dashboard.dashboards.instances.instances import\
    console as i_console
from openstack_dashboard.dashboards.instances.instances import\
    views as i_views
from openstack_dashboard.dashboards.instances.instances.workflows import\
    create_instance as i_workflows
from openstack_dashboard.dashboards.network.networks import\
    views as n_views
from openstack_dashboard.dashboards.network.networks import\
    workflows as n_workflows
from openstack_dashboard.dashboards.network.routers import\
    views as r_views


class NTCreateRouterView(r_views.CreateView):
    template_name = 'network/network_topology/create_router.html'
    success_url = reverse_lazy("horizon:network:network_topology:index")
    page_title = _("Create a Router")


class NTCreateNetwork(n_workflows.CreateNetwork):
    def get_success_url(self):
        return reverse("horizon:network:network_topology:index")
...
class NTLaunchInstance(i_workflows.LaunchInstance):
    success_url = "horizon:network:network_topology:index"
...
class InstanceView(i_views.IndexView):
    ...
    template_name = 'network/network_topology/iframe.html'
...
class NetworkTopologyView(views.HorizonTemplateView):
    template_name = 'network/network_topology/index.html'
...
class JSONView(View):
    ...
    def _get_servers(self, request):
        ...
        self.add_resource_url('horizon:instances:instances:detail', data)
        return data
    ...
    def _get_networks(self, request):
        ...
        self.add_resource_url('horizon:network:networks:detail',
                              networks)
    ...
    def _get_routers(self, request):
        ...
        self.add_resource_url('horizon:network:routers:detail', routers)
        return routers
    ...
    def _get_ports(self, request):
        ...
        self.add_resource_url('horizon:network:networks:ports:detail',
                              ports)
        return ports
