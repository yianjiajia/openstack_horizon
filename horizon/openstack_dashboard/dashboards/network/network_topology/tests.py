...
JSON_URL = reverse('horizon:network:network_topology:json')
INDEX_URL = reverse('horizon:network:network_topology:index')
    ...
    def _test_json_view(self, router_enable=True):
        ...
        expect_server_urls = [
            {'id': server.id,
             ...
             'task': None,
             'url': '/network/instances/%s/' % server.id}
        ...
        if router_enable:
            expect_router_urls = [
                {'id': router.id,
                 ...
                 'url': '/network/routers/%s/' % router.id}
        ...
        expect_net_urls += [{'id': net.id,
                             'url': '/network/networks/%s/detail' % net.id,
                             'name': net.name,
                             'router:external': net.router__external,
                             'subnets': [{'cidr': subnet.cidr}
                                         for subnet in net.subnets]}
        ...
        expect_port_urls = [
            {'id': port.id,
             ...
             'status': port.status,
             'url': '/network/networks/ports/%s/detail' % port.id}
...
class NetworkTopologyCreateTests(test.TestCase):
    ...
    def _test_new_button_disabled_when_quota_exceeded(
        ...
        self.assertTemplateUsed(res, 'network/network_topology/index.html')
    ...
    def test_create_network_button_disabled_when_quota_exceeded(self):
        url = reverse('horizon:network:network_topology:createnetwork')
    ...
    def test_create_router_button_disabled_when_quota_exceeded(self):
        url = reverse('horizon:network:network_topology:createrouter')
        classes = 'btn btn-default btn-sm ajax-modal'
    ...
    def test_launch_instance_button_disabled_when_quota_exceeded(self):
        url = reverse('horizon:network:network_topology:launchinstance')
