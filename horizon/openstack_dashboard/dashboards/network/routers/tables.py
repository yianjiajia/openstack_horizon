...
class DeleteRouter(policy.PolicyTargetMixin, tables.DeleteAction):
    ...
    def delete(self, request, obj_id):
        try:
            ...
            # operation log
            config = _('Route ID: %s') %obj_id

            api.logger.Logger(request).create(resource_type='route', action_name='Delete Route',
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='route', action_name='Delete Route',
                                              resource_name='Route', config=_('Unable to delete router "%s"') % e, status='Error')
        except Exception:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='route', action_name='Delete Route',
                                              resource_name='Route', config=_('Unable to delete router "%s"') % name,
                                                       status='Error')
...
class CreateRouter(tables.LinkAction):
    ...
    def allowed(self, request, datum=None):
        ...
        if account_is_frozen:
            ...
            if not (usages['routers']['available'] <= 0):
                self.classes = [c for c in self.classes if c != "disabled"]
        return True
...
class EditRouter(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    def allowed(self, request, datum):
        ...
        if account_is_frozen:
            ...
            self.classes = [c for c in self.classes if c != "disabled"]
        return super(EditRouter, self).allowed(request, datum)
...
class ClearGateway(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        ...
        try:
            ...
            api.logger.Logger(request).create(resource_type='route', action_name='Delete Gateway',
                                                       resource_name='Route', config=config,
                                                       status='Success')
...
class RoutersTable(tables.DataTable):
    ...
    bandwidth = tables.Column("bandwidth", verbose_name=_("Bandwidth (Mbps)"))
    ...
    class Meta(object):
        ...
        table_actions = (CreateRouter, DeleteRouter,
                         RoutersFilterAction)
        row_actions = (EditRouter, SetGateway, ClearGateway, DeleteRouter)
