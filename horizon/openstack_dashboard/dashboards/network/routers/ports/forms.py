...
class AddInterface(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        # operation log
        config = _('IP Address: %s') %data['ip_address']

        api.logger.Logger(request).create(resource_type='route', action_name='Add Interface',
    ...
    def _handle_error(self, request, router_id, reason):
        ...
        # operation log
        api.logger.Logger(request).create(resource_type='route', action_name='Add Interface',
                                                       resource_name='Route', config=_('Failed to add_interface: %s') % reason,
                                                       status='Error')
...
class SetGatewayForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            config = _("Route ID: %s Network ID: %s") %(data['router_id'], data['network_id'])
            api.logger.Logger(request).create(resource_type='route', action_name='Add Gateway',
                                                       resource_name='Route', config=config,
                                                       status='Success')
            return True
        except Exception as e:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='route', action_name='Add Gateway',
                                                       resource_name='Route', config=_('Failed to set gateway %s') % e,
                                                       status='Error')
