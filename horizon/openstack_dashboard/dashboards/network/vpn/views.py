...
from openstack_dashboard.utils import vpncredential as vpncredentialUtil
...
class IndexView(tabs.TabView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if m == 'vpnservice':
            ...
            for obj_id in obj_ids:
                try:
                    ...
                    # operation log
                    config = _('VPN Service ID: %s') %obj_id
                    api.logger.Logger(request).create(resource_type='vpn', action_name='Remove VPN Service',
                    ...
                    # operation log
                    api.logger.Logger(request).create(resource_type='vpn', action_name='Remove VPN Service',
                                                       resource_name='vpn', config= _('Unable to delete VPN Service: %s')
                                      % e,
                                                       status='Error')
        elif m == 'ikepolicy':
            for obj_id in obj_ids:
        ...
        elif m == 'vpncredential':
            for obj_id in obj_ids:
                try:
                    api.vpn.vpncredential_delete(request, obj_id)
                    messages.success(request,
                                     _('Deleted VPN Credential %s') % obj_id)

                    # operation log
                    config = _('VPN Credential ID: %s') %obj_id
                    api.logger.Logger(request).create(resource_type='vpn', action_name='Delete VPN Credential',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
                except Exception as e:
                    exceptions.handle(request,
                                      _('Unable to delete VPN Credential: %s') % e)

                     # operation log
                    api.logger.Logger(request).create(resource_type='vpn', action_name='Delete VPN Credential',
                                                       resource_name='vpn', config=_('Unable to delete VPN Credential: %s') % e,
                                                       status='Error')
        elif m == 'sslvpnconnection':
            for obj_id in obj_ids:
                try:
                    api.vpn.sslvpnconnection_delete(request, obj_id)
                    messages.success(request,
                                     _('Deleted SSL VPN Connection %s')
                                     % obj_id)
                    # operation log
                    config = _('VPN Connection ID: %s') %obj_id
                    api.logger.Logger(request).create(resource_type='vpn', action_name='Delete VPN Connection',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
                except Exception as e:
                    exceptions.handle(request,
                        _('Unable to delete SSL VPN Connection: %s') % e)

                     # operation log
                    api.logger.Logger(request).create(resource_type='vpn', action_name='Delete VPN Connection',
                                                       resource_name='vpn', config= _('Unable to delete SSL VPN Connection: %s') % e,
                                                       status='Error')

        return self.get(request, *args, **kwargs)

class AddVPNCredentialView(workflows.WorkflowView):
    workflow_class = vpn_workflows.AddVPNCredential

    def get_initial(self):
        initial = super(AddVPNCredentialView, self).get_initial()
        return initial

class AddSSLVPNConnectionView(workflows.WorkflowView):
    workflow_class = vpn_workflows.AddSSLVPNConnection

    def get_initial(self):
        initial = super(AddSSLVPNConnectionView, self).get_initial()
        return initial

class VPNCredentialDetailsView(tabs.TabView):
    tab_group_class = (vpn_tabs.VPNCredentialDetailsTabs)
    template_name = 'network/vpn/details_tabs.html'

class SSLVPNConnectionDetailsView(tabs.TabView):
    tab_group_class = (vpn_tabs.SSLVPNConnectionDetailsTabs)
    template_name = 'network/vpn/details_tabs.html'

class UpdateVPNCredentialView(forms.ModalFormView):
    form_class = vpn_forms.UpdateVPNCredential
    template_name = "network/vpn/update_vpncredential.html"
    context_object_name = 'vpncredential'
    success_url = reverse_lazy("horizon:network:vpn:index")

    def get_context_data(self, **kwargs):
        context = super(UpdateVPNCredentialView, self).get_context_data(**kwargs)
        context["vpncredential_id"] = self.kwargs['vpncredential_id']
        return context

    @memoized.memoized_method
    def _get_object(self, *args, **kwargs):
        vpncredential_id = self.kwargs['vpncredential_id']
        try:
            vpncredential = api.vpn.vpncredential_get(self.request, vpncredential_id)
            return vpncredential
        except Exception as e:
            redirect = self.success_url
            msg = _('Unable to retrieve VPN Credential details. %s') % e
            exceptions.handle(self.request, msg, redirect=redirect)

    def get_initial(self):
        vpncredential = self._get_object()
        return {'name': vpncredential['name'],
                'vpncredential_id': vpncredential['id'],
                'description': vpncredential['description'],}

class UpdateSSLVPNConnectionView(forms.ModalFormView):
    form_class = vpn_forms.UpdateSSLVPNConnection
    template_name = "network/vpn/update_sslvpnconnection.html"
    context_object_name = 'sslvpnconnection'
    success_url = reverse_lazy("horizon:network:vpn:index")

    def get_context_data(self, **kwargs):
        context = super(
            UpdateSSLVPNConnectionView, self).get_context_data(**kwargs)
        context["sslvpnconnection_id"] = self.kwargs[
            'sslvpnconnection_id']
        return context

    @memoized.memoized_method
    def _get_object(self, *args, **kwargs):
        connection_id = self.kwargs['sslvpnconnection_id']
        try:
            return api.vpn.sslvpnconnection_get(self.request, connection_id)
        except Exception as e:
            redirect = self.success_url
            msg = _('Unable to retrieve SSL VPN Connection details. %s') % e
            exceptions.handle(self.request, msg, redirect=redirect)

    def get_initial(self):
        sslvpnconnection = self._get_object()
        return {'name': sslvpnconnection['name'],
                'sslvpnconnection_id': sslvpnconnection['id'],
                'description': sslvpnconnection['description'],
                'admin_state_up': sslvpnconnection['admin_state_up']}
        
class DownloadOvpnFileView(View):
    def get(self, request, sslvpnconnection_id=None):
        try:
            sslvpnconnection=api.vpn.sslvpnconnection_get(self.request, sslvpnconnection_id)
            vpncredential=sslvpnconnection.vpncredential
            router_id=sslvpnconnection.vpnservice.router_id
            ports = api.neutron.port_list(request, device_id=router_id)
            for port in ports:
                if port.device_owner=="network:router_gateway":
                    server=port.fixed_ips[0]["ip_address"]
            content = vpncredentialUtil.renderOvpn("/etc/openstack-dashboard",
                server_addr=server,
                ca_crt=vpncredential.ca,
                client_crt=vpncredential.client_certificate,
                client_key=vpncredential.client_key)

            # operation log

            api.logger.Logger(request).create(resource_type='vpn', action_name='Download OVPN File',
                                                       resource_name='vpn', config='',
                                                       status='Success')
        except Exception:
            redirect = reverse_lazy("horizon:network:vpn:index")
            exceptions.handle(self.request,
                          _('Unable to download the ovpn file'),
                          redirect=redirect)
        ...
        response = http.HttpResponse(content_type='application/ovpn')
        response['Content-Disposition'] = \
                'attachment; filename="%s.ovpn"' % sslvpnconnection_id
