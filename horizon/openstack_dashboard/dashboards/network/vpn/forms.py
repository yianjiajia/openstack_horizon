...
class UpdateVPNCredential(forms.SelfHandlingForm):
    vpncredential_id = forms.CharField(
    ...
    def handle(self, request, context):
        try:
            ...
            vpncredential = api.vpn.vpncredential_update(
                request, context['vpncredential_id'], **data)
            msg = (_('SSL VPN Credential %s was successfully updated.')
                   % context['name'])
            LOG.debug(msg)
            messages.success(request, msg)

            # operation log
            config = _('Credential Name: %s') %context['name']

            api.logger.Logger(request).create(resource_type='vpn', action_name='Update VPN Credential',
                                                       resource_name='vpn', config=config,
                                                       status='Success')

            return vpncredential
        except Exception as e:
            msg = _('Failed to update VPN Credential %s') % context['name']
            LOG.info('%s: %s' % (msg, e))
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='vpn', action_name='Update VPN Credential',
                                                       resource_name='vpn', config=_('Failed to update VPN Credential %s') % context['name'],
                                                       status='Error')

class UpdateSSLVPNConnection(forms.SelfHandlingForm):
    sslvpnconnection_id = forms.CharField(label=_("ID"),
    ...
    def handle(self, request, context):
        try:
            ...
            sslvpnconnection = api.vpn.sslvpnconnection_update(
                request, context['sslvpnconnection_id'], **data)
            msg = (_('SSL VPN Connection %s was successfully updated.')
                   % context['name'])
            LOG.debug(msg)
            messages.success(request, msg)

            # operation log
            config = _('VPN Connection Name: %s') %context['name']

            api.logger.Logger(request).create(resource_type='vpn', action_name='Update VPN Connection',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
            return sslvpnconnection
        except Exception as e:
            msg = (_('Failed to update SSL VPN Connection %s')
                   % context['name'])
            LOG.info('%s: %s' % (msg, e))
            redirect = reverse(self.failure_url)
            exceptions.handle(request, msg, redirect=redirect)\
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='vpn', action_name='Update VPN Connection',
                                                       resource_name='vpn', config=_('Failed to update SSL '
                                                                                     'VPN Connection %s') % context['name'],
                                                       status='Error')
...
class UpdateVPNService(forms.SelfHandlingForm):
    ...
    def handle(self, request, context):
        ...
        try:
            ...
            api.logger.Logger(request).create(resource_type='vpn', action_name='Update VPN Service',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
            return vpnservice
        except Exception as e:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='vpn', action_name='Update VPN Service',
                                                       resource_name='vpn', config=_('Failed to update VPN Service %s') % context['name'],
                                                       status='Error')
...
class UpdateIPSecSiteConnection(forms.SelfHandlingForm):
    ...
    def handle(self, request, context):
        ...
        try:
            ...
            # operation log
            config = _('IPSec Site Connection Name: %s') %context['name']
            api.logger.Logger(request).create(resource_type='vpn', action_name=' Update IPSec Site Connection',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='vpn', action_name=' Update IPSec Site Connection',
                                                       resource_name='vpn', config=_('Failed to update IPSec '
                                                                                     'Site Connection %s')% context['name'],
                                                       status='Error')
