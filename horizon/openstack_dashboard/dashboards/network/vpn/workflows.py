...
class AddVPNCredentialAction(workflows.Action):
...
#                                         initial=True, required=False)
    ...
    def __init__(self, request, *args, **kwargs):
        super(AddVPNCredentialAction, self).__init__(request, *args, **kwargs)

    class Meta:
        name = _("Add VPN Credential")
        permissions = ('openstack.services.network',)
        help_text = _("Create VPN Credential for current project.\n\n"
                      "Assign a name and description for the VPN Credential. "
                      #"Admin State is Up (checked) by default."
                      )


class AddVPNCredentialStep(workflows.Step):
    action_class = AddVPNCredentialAction
    contributes = ("name", "description")

    def contribute(self, data, context):
        context = super(AddVPNCredentialStep, self).contribute(data, context)
        if data:
            return context


class AddVPNCredential(workflows.Workflow):
    slug = "addvpncredential"
    name = _("Add VPN Credential")
    finalize_button_name = _("Add")
    success_message = _('Added VPN Credential "%s".')
    failure_message = _('Unable to add VPN Credential "%s".')
    success_url = "horizon:network:vpn:index"
    default_steps = (AddVPNCredentialStep,)
    ...
    def handle(self, request, context):
        try:
            ...
            if result:
                ...
                # operation log
                config = _('VPN Credential Name: %s') %context['name']

                api.logger.Logger(request).create(resource_type='vpn', action_name='Add Vpn Credential',
            ...
            exceptions.handle(request,
                              _('Unable to create VPN credential.'))

            # operation log
            api.logger.Logger(request).create(resource_type='vpn', action_name='Add Vpn Credential',
                                                       resource_name='vpn', config= _('Unable to create VPN credential.'),
                                                       status='Error')
            return False

class AddSSLVPNConnectionAction(workflows.Action):
    name = forms.CharField(max_length=80, label=_("Name"))
    description = forms.CharField(
        initial="", required=False,
        max_length=80, label=_("Description"))
    credential_id = forms.ChoiceField(label=_("SSL VPN Credential"))
    vpnservice_id = forms.ChoiceField(
        label=_("VPN Service associated with this connection"))
    client_address_pool_cidr=forms.CharField(
        initial="192.168.80.0/24",
        max_length=80, label=_("Client Address Pool Cidr"),
        help_text=_("Remote peer ppp(s) address(es) with mask(s) in CIDR format "
                    "(e.g. 192.168.80.0/24)"))
    admin_state_up = forms.BooleanField(label=_("Admin State"),
                                        initial=True, required=False)

    def __init__(self, request, *args, **kwargs):
        super(AddSSLVPNConnectionAction, self).__init__(request, *args, **kwargs)

    def populate_credential_id_choices(self, request, context):
        credential_id_choices = [('', _("Select a VPN Credential"))]
        try:
        ...
        for s in vpncredentials:
            credential_id_choices.append((s.id, s.name))
        self.fields['credential_id'].choices = credential_id_choices
        return credential_id_choices
    ...
    class Meta:
        name = _("Add SSL VPN Connection")
        permissions = ('openstack.services.network',)
        slug = 'add_ssl_vpn'
        help_text = _("Create SSL VPN Connection for current project.\n\n"
                      "Assign a name and description and client ppp cidr for the "
                      "SSL VPN Connection. "
                      "All fields in this tab are required."
                      )


class AddSSLVPNConnectionStep(workflows.Step):
    action_class = AddSSLVPNConnectionAction
    contributes = ("name", "description", "credential_id",
                   "vpnservice_id", "client_address_pool_cidr", "admin_state_up")


    def contribute(self, data, context):
        context = super(AddSSLVPNConnectionStep, self).contribute(data, context)
        if data:
            return context


class AddSSLVPNConnection(workflows.Workflow):
    slug = "addsslvpnconection"
    name = _("Add SSL VPN Connection")
    finalize_button_name = _("Add")
    success_message = _('Added SSL VPN Connection "%s".')
    failure_message = _('Unable to add SSL VPN Connection "%s".')
    success_url = "horizon:network:vpn:index"
    default_steps = (AddSSLVPNConnectionStep,)
    ...
    def handle(self, request, context):
        try:
            ...
            # operation log
            config = _('VPN SSLVPNConnection Name: %s') %context['name']

            api.logger.Logger(request).create(resource_type='vpn', action_name='Add SSL VPN Connection',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
            return True
        except Exception:
...
class AddVPNService(workflows.Workflow):
    ...
    def handle(self, request, context):
        try:
            ...
            # operation log
            config = _('VPN Service Name: %s') % context['name']
            api.logger.Logger(request).create(resource_type='vpn', action_name='Add VPN Service',
                                                       resource_name='vpn', config=config,
                                                       status='Success')
            return True
        except Exception:
