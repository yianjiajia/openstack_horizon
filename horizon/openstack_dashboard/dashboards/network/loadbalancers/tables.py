...
class AddLoadBalanderLink(tables.LinkAction):
    name = "addloadbalander"
    verbose_name = _("Add LoadBalander")
    url = "horizon:network:loadbalancers:addloadbalander"
    classes = ("ajax-modal",)
    icon = "plus"
    ...
    def allowed(self, request, datum=None):
        ...
        if usages['pool']['available'] <= 0:
            if "disabled" not in self.classes:
                self.classes = [c for c in self.classes] + ["disabled"]
                self.verbose_name = _("Add LoadBalander (Quota exceeded)")
        else:
            self.verbose_name = _("Add LoadBalander")
            self.classes = [c for c in self.classes if c != "disabled"]

        return True


class DeletLoadBalanderLink(policy.PolicyTargetMixin, tables.DeleteAction):
    ...
    def action_present(count):
        return ungettext_lazy(
            u"Delete LoadBalander",
            u"Delete LoadBalanders",
            count
        )
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Scheduled deletion of LoadBalander",
            u"Scheduled deletion of LoadBalanders",
            count
        )


class BackendServerLink(tables.LinkAction):
    name = "backendserver"
    verbose_name = _("Backend Server")
    url = "horizon:network:loadbalancers:backendserver"
    classes = ("ajax-modal",)
    icon = "plus"
    policy_rules = (("network", "create_loadbalancer"),)

    def get_link_url(self, pool):
        base_url = reverse("horizon:network:loadbalancers:addbackendserver",
                           kwargs={'pool_id': pool.id})
        return base_url


class UpdateLoadBalancerRow(tables.Row):
    ajax = True

    def get_data(self, request, pool_id):
        pool = api.lbaas.pool_get(request, pool_id)
        vip = api.lbaas.vip_get(request, pool.vip_id)
        pool.subnet_name = pool.subnet.cidr
        pool.internal_ip = vip.address
        fips = api.network.tenant_floating_ip_list(request)
        vip_fip = [fip for fip in fips
                   if fip.port_id == vip.port_id]
        if vip_fip:
            pool.floating_ip = vip_fip[0].floating_ip_address
        return pool
...
class UpdateLoadBalancerLink(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    def get_link_url(self, pool):
        base_url = reverse("horizon:network:loadbalancers:updateloadbalancer",
                           kwargs={'pool_id': pool.id})
        return base_url
