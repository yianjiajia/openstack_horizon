...
class AddPool(workflows.Workflow):
    ...
    def handle(self, request, context):
        try:
            ...
            api.logger.Logger(request).create(resource_type='loadblance', action_name='Create Loadblance',
...
#     choices=[('private_network', _('Private Network')),
#              ('base_network', _('Base Network'))],
#     widget=forms.RadioSelect)
    subnet_id = forms.ChoiceField(
        label=_("Subnet"),
        widget=forms.Select)
#     set_floatingip = forms.BooleanField(label=_("Set Floatingip"))
#     band_width = forms.CharField(label=_("Band Width"), required=False)
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        for mode in ('SOURCE_IP', 'HTTP_COOKIE', 'APP_COOKIE'):
            session_persistence_choices.append((mode.lower(), mode))
        self.fields['session_persistence'].choices = session_persistence_choices

        if len(request.POST) > 1 and ('set_health_check' in request.POST and  request.POST['set_health_check'] != 'True' ):
            self.fields['type'].required = False
            self.fields['delay'].required = False
            self.fields['timeout'].required = False
            self.fields['max_retries'].required = False

    def clean(self):
        cleaned_data = super(AddLoadBalancerAction, self).clean()
        persistence = cleaned_data.get('session_persistence')
        if persistence:
            cleaned_data['session_persistence'] = persistence.upper()
        if (cleaned_data.get('session_persistence') == 'APP_COOKIE' and
...
class AddLoadBalancerStep(workflows.Step):
    ...
    contributes = ("name", "network_type", "set_floatingip", "band_width", "subnet_id",
                   "listener_name", "protocol", "lb_method", "protocol_port", "set_health_check",
                    "type", "delay", "timeout", "max_retries","url_path", "http_method",  "expected_codes",
                   "admin_state_up","address", "session_persistence", "cookie_name",
                   "connection_limit")
    def contribute(self, data, context):
...
class AddLoadBalancer(workflows.Workflow):
    ...
    def handle(self, request, context):
        ...
        try:
            ...
            exceptions.handle(request, _("Unable to add LoadBalancer."))
        return False


class MultipleChoiceFieldNotVerifyChoice(forms.MultipleChoiceField):
    def validate(self, value):
        pass


class BackendServerAction(workflows.MembershipAction):
    protocol_port = forms.IntegerField(label = _('Port'), min_value=1, max_value=65535, required=False,
                                       widget = forms.TextInput(attrs = {'style' : 'width: 10%'}),
                                       help_text=_("Enter an integer value between 1 and 65535. "
                                                    "The same port will be used for all the selected "
                                                    "members and can be modified later."))
    weight = forms.IntegerField(label = _('Weight'), min_value=1, max_value=256, required=False,
                                widget = forms.TextInput(attrs = {'style' : 'width: 10%'}),
                                help_text=_("Relative part of requests this pool member serves "
                                            "compared to others. \nThe same weight will be applied to "
                                            "all the selected members and can be modified later. "
                                            "Weight must be in the range 1 to 256."))
    def __init__(self, request, *args, **kwargs):
        super(BackendServerAction, self).__init__(request,
                                                         *args,
                                                         **kwargs)
        err_msg = _('Unable to retrieve backend server list. Please try again later.')
        ...
        # get pool to see member in the pool
        try:
            pool = api.lbaas.pool_get(request, args[0]['pool_id'])
        except Exception:
            exceptions.handle(request, _('Unable to retrieve member list.'))
        servers_list = []
        new_servers = []
        for server in servers:
            params = {'device_id': server.id}
            try:
                plist = api.neutron.port_list(request, **params)
            except Exception:
                return False
            address = plist[0].fixed_ips[0]['ip_address'] #TODO if have multiple fixed_ips
            # only add server have the same network as the pool
            if plist[0].network_id == pool.subnet.network_id:
                new_servers.append({'name':server.name, 'address' :address, 'status': server.status})
        role = {'id':'default', 'name': 'default'}
        field_name = self.get_member_field_name(role['id'])
        label = role['name']
        self.fields[field_name] = MultipleChoiceFieldNotVerifyChoice(required=False,
                                                            label=label)
        member_in_pool = []
        addree_in_pool = []
        for ns in new_servers:
            for m in pool.members:
                if m.address == ns['address']:
                    addree_in_pool.append(m.address)
                    member_in_pool.append(m.address + (':Port:%s:Weight:%s') % ( m.protocol_port, m.weight))
                    servers_list.append((m.address + (':Port:%s:Weight:%s') % ( m.protocol_port, m.weight), ('%s:%s:Port:%s:Weight:%s:%s') % 
                                         (ns['name'], m.address, m.protocol_port, m.weight, ns['status'])))
        left_ns = [ns for ns in new_servers if ns['address'] not in addree_in_pool]
        servers_list.extend([(n['address'], ('%s:%s:%s') % (n['name'], n['address'], n['status'])) for n in left_ns])
        self.fields[field_name].choices = servers_list
        self.fields[field_name].initial = member_in_pool

    class Meta(object):
        name = _("Backend Servers")
        slug = 'backendserver'


class BackendServerStep(workflows.UpdateMembersStep):
    action_class = BackendServerAction
    available_list_title = _("All servers")
    members_list_title = _("Selected Backend Servers")
    no_available_text = _("No servers found.")
    no_members_text = _("No servers.")
    template_name = "network/loadbalancers/_workflow_step_update_servers.html"
    depends_on = ('pool_id',)
    contributes = ("backendserver_role_default",)
    show_roles = False


class BackendServer(workflows.Workflow):
    slug = "backend_server"
    name = _("Backend Server")
    success_message = _('Updated backend server.')
    failure_message = _('Unable to Update backend server.')
    success_url = "horizon:network:loadbalancers:index"
    default_steps = (BackendServerStep,)
    ...
    def handle(self, request, context):
        try:
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='loadblance', action_name='Manager Backserver',
                                                       resource_name='Loadblance', config= _('Unable to retrieve '
                                     'the specified pool.'),
                                                       status='Error')
            return False
        # first delete all member for the pool
        for obj in pool.members:
            try:
                api.lbaas.member_delete(request, obj.id)
            except Exception as e:
                exceptions.handle(request,
                ...
                api.logger.Logger(request).create(resource_type='loadblance', action_name='Manager Backserver',
                                     ...
                                     'the specified pool.'),
                                                       status='Error')
        # add new member from input
        for m in context['backendserver_role_default']:
            # 192.168.1.62:Port:80:Weight:62
                address, temp1, protocol_port, temp2, weight = m.split(':')
                ...
                try:
                    ...
                    # operation log
                    api.logger.Logger(request).create(resource_type='loadblance', action_name='Manager Backserver',
                                                       resource_name='Loadblance', config=_('Unable to retrieve the specified pool.'),
                                                       status='Error')
                    return False
        # operation log
        api.logger.Logger(request).create(resource_type='loadblance', action_name='Manager Backserver',
