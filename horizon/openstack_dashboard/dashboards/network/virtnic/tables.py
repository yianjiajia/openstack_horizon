...
class CreateVirtnic(tables.LinkAction):
    ...
    url = "horizon:network:virtnic:create"
    classes = ("ajax-modal",)
    icon = "plus"
...
class DeleteVirtnic(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        ...
        # operation log
        config = _('Port ID: %s') %obj_id

        api.logger.Logger(request).create(resource_type='virtnic', action_name='Delete Virtual Network Card',
...
class BindingVirtnic(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:network:routers:binding"
    classes = ("ajax-modal",)
    icon = "pencil"
    ...
    def get_link_url(self, port):
        base_url = reverse("horizon:network:virtnic:binding",
...
class unBindingVirtnic(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    redirect_url="horizon:network:virtnic:index"
    ...
    def allowed(self, request, instance=None):
        if instance and hasattr(instance, "device_owner"):
            if instance["device_owner"] in ("compute:nova", "compute:None") and hasattr(instance, "device_id"):
                if len(instance["device_id"])>0:
                    return True
        return False
    ...
    def action(self, request, obj_id):
        ...
        if hasattr(port, "device_id"):
            ...
            # operation log
            config = _('Virtnic ID: %s') %obj_id

            api.logger.Logger(request).create(resource_type='virtnic', action_name='Unbinding Virtual Network Card',
                                                       resource_name='Virtual Network Card', config=config,
                                                       status='Success')
            import time
            time.sleep(3)
...
class VirtnicTable(tables.DataTable):
    name = tables.Column("name_or_id",
                         link="horizon:network:virtnic:detail",
                         verbose_name=_("Name"))
