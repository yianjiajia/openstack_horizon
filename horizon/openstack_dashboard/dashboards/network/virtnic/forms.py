...
class CreateForm(forms.SelfHandlingForm):
    ...
    subnetwork = forms.ChoiceField(label=_("Subnetwork"),
                                         required=True)
    failure_url = 'horizon:network:virtnic:index'
...
#         if choices:
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            config = _('Virtnic Name: %s') %data['name']

            api.logger.Logger(request).create(resource_type='virtnic', action_name='Create Virtual Network Card',
                                                       resource_name='Virtual Network Card', config=config,
                                                       status='Success')
            return port
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='virtnic', action_name='Create Virtual Network Card',
                                                       resource_name='Virtual Network Card', config=_('Failed to create virtic %s.') % data['name'],
                                                       status='Error')
            return False
...
class BindingForm(forms.SelfHandlingForm):
    ...
    redirect_url = reverse_lazy('horizon:network:virtnic:index')
...
#         if choices:
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            config = _("Virtnic Name: %s Port ID: %s Instance ID: %s") %(data['name'], port_id, instance_id)

            api.logger.Logger(request).create(resource_type='virtnic', action_name='Bind Virtual Network Card',
                                                       resource_name='Virtual Network Card', config=config,
                                                       status='Success')
            ...
            exceptions.handle(request, msg, redirect=self.redirect_url)
             ...
             # operation log
            api.logger.Logger(request).create(resource_type='virtnic', action_name='Bind Virtual Network Card',
                                                       resource_name='Virtual Network Card', config=_('Failed to binding %s') % data['name'],
                                                       status='Error')
