...
ALLOCATE_URL = "horizon:network:floating_ips:allocate"
...
class AssociateIPAction(workflows.Action):
    ...
    def populate_ip_id_choices(self, request, context):
        ips = []
        redirect = reverse('horizon:network:floating_ips:index')
        try:
...
class IPAssociationWorkflow(workflows.Workflow):
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            resource_type = 'instance' if request.POST.get('next') else 'floatip'
            resource_name = 'Instance' if request.POST.get('next') else 'Floating IP'
            config = _("Floating IP ID: %s Instance ID: %s") %(data['ip_id'], data['instance_id'])
            api.logger.Logger(request).create(resource_type=resource_type, action_name='Associated Floating IP',
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='floatip', action_name='Associated Floating IP',
                                                       resource_name='Floating IP', config=_('The requested instance port is already'
                                                                                            ' associated with another floating IP.'),
                                                           status='Error')
            return False
