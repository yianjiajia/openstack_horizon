...
class DeleteNetwork(policy.PolicyTargetMixin, CheckNetworkEditable,
    ...
    def delete(self, request, network_id):
        ...
        try:
            ...
            # operation log
            config = _('Network ID: %s') %network_id

            api.logger.Logger(request).create(resource_type='network', action_name='Remove Network',
            ...
            api.logger.Logger(request).create(resource_type='network', action_name='Remove Network',
                                                       resource_name='Network', config=_('Failed to delete network %s') %network_id,
                                                       status='Error')
...
class CreateNetwork(tables.LinkAction):
    ...
    url = "horizon:network:networks:create"
    classes = ("ajax-modal",)
    icon = "plus"
...
class EditNetwork(policy.PolicyTargetMixin, CheckNetworkEditable,
    ...
    url = "horizon:network:networks:update"
    classes = ("ajax-modal",)
    icon = "pencil"
    ...
    def allowed(self, request, datum=None):
        ...
        if account_is_frozen:
            ...
            self.classes = [c for c in self.classes if c != "disabled"]
        return super(EditNetwork, self).allowed(request, datum)
...
def get_subnets(network):
    template_name = 'network/networks/_network_ips.html'
...
class NetworksTable(tables.DataTable):
    name = tables.Column("name_or_id",
                         verbose_name=_("Name"),
                         link='horizon:network:networks:detail')
