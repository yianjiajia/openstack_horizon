...
class UpdatePort(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Port ID: %s Port Name: %s") %(data['port_id'], data['name'])
            api.logger.Logger(request).create(resource_type='network', action_name='Update Port',
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='network', action_name='Update Port',
                                                       resource_name='Network', config= _('Failed to update port %s') % data['port_id'],
                                                       status='Error')
