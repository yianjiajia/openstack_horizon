...
class UpdateSubnet(network_workflows.CreateNetwork):
    ...
    def _update_subnet(self, request, data):
        ...
        try:
            ...
            # operation log
            config = '\n'.join(['Subnet ID: '+ subnet_id, 'Subnet Name: '+ data['subnet_name']])

            api.logger.Logger(request).create(resource_type='network', action_name='Update Subnet',
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='network', action_name='Update Subnet',
                                                       resource_name='Network', config='Failed to update subnet',
                                                       status='Error')
            return False
