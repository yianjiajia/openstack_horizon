...
class DeleteSubnet(SubnetPolicyTargetMixin, CheckNetworkEditable,
    ...
    def delete(self, request, obj_id):
        try:
            ...
            # operation log
            config = _('Subnet ID: %s') %obj_id

            api.logger.Logger(request).create(resource_type='network', action_name='Remove Subnet',
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='network', action_name='Remove Subnet',
                                                       resource_name='Network', config=_('Failed to delete subnet %s') % obj_id,
                                                       status='Error')
...
class UpdateSubnet(SubnetPolicyTargetMixin, CheckNetworkEditable,
    ...
    def allowed(self, request, datum=None):
        ...
        if account_is_frozen:
            ...
            self.classes = [c for c in self.classes if c != "disabled"]
        return super(UpdateSubnet, self).allowed(request, datum)
