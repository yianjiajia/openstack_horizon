...
from openstack_dashboard.dashboards.network.networks.subnets import utils
...
class CreateNetwork(workflows.Workflow):
    ...
    def _create_subnet(self, request, data, network=None, tenant_id=None,
        ...
        try:
            ...
            # operation log
            config = _("Subnet ID: %s Subnet Name: %s") %(subnet.id, data['subnet_name'])
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='network', action_name='Create Subnet',
                                                       resource_name='Network', config=_('Failed to create subnet'),
                                                       status='Error')
            return False
    ...
    def handle(self, request, data):
        ...
        if not network:
            ...
            api.logger.Logger(request).create(resource_type='network', action_name='Create Network',
                                                       resource_name='Network', config=_('Failed to create network'),
                                                       status='Error')
            return False

        # operation log
        config = _("Network Name: %s On/Off: %s") %(data.get('net_name'), data.get('admin_state'))
        api.logger.Logger(request).create(resource_type='network', action_name='Create Network',
                                                       resource_name='Network', config=config,
                                                       status='Success')
        ...
        if subnet:
            ...
            # operation log
            config = _("Network Name: %s On/Off: %s Subnet Name: %s Net Address: %s") %(data.get('net_name'), data.get('admin_state'),
                                                                                     data.get('subnet_name'), data.get('cidr'))
            api.logger.Logger(request).create(resource_type='network', action_name='Create Network',
                                                       resource_name='Network', config=config,
                                                       status='Success')
            return True
