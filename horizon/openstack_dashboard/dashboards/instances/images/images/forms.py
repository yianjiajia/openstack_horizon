...
class CreateImageForm(forms.SelfHandlingForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if not request.user.is_superuser:
            self.fields['is_public'].widget = forms.widgets.HiddenInput()
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Image Name: %s Disk Format: %s Public: %s") %(data['name'], data['disk_format'], str(data['is_public']))
            api.logger.Logger(request).create(resource_type='image', action_name='Create Image',
                                                       resource_name='Image', config=config,
                                                       status='Success')
            return image
            ...
            exceptions.handle(request, msg)
             ...
             # operation log
            api.logger.Logger(request).create(resource_type='image', action_name='Create Image',
                                                       resource_name='Image', config=msg,
                                                       status='Error')
...
class UpdateImageForm(forms.SelfHandlingForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if not request.user.is_superuser:
            self.fields['public'].widget = forms.widgets.HiddenInput()
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _("Image ID: %s Disk Format: %s Public: %s") % (data['image_id'], data['disk_format'],
                                                                  str(data['public']))
            api.logger.Logger(request).create(resource_type='image', action_name='Update Image',
                                                       resource_name='Image', config=config,
                                                       status='Success')
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='image', action_name='Update Image',
                                                       resource_name='Image', config=error_updating % image_id,
                                                       status='Error')