...
from openstack_dashboard.dashboards.project.floating_ips \
    import workflows
from openstack_dashboard.dashboards.instances.instances import tabs
...
class ResetInstanceActive(policy.PolicyTargetMixin, tables.BatchAction):
    name = "reset active"
    classes = ("btn-danger",)
    icon = "refresh"
    policy_rules = (("compute","compute:reset"),)
    help_text = _("Reset instances who are error status to active status.")
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Scheduled Reset of Instance",
            u"Scheduled Reset of Instances",
            count
        )

    def allowed(self, request, instance=None):
        if instance is not None:
            return ((instance.status in ERROR_STATES)
                    and not is_deleting(instance))
        else:
            return False

    def action(self, request, obj_id):
        api.nova.server_reset_active(request, obj_id)
...
class TerminateInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id, del_system_disk, del_data_disk):
        api.nova.server_delete(request, obj_id, del_system_disk, del_data_disk)
        # operation log
        api.logger.Logger(request).create(resource_type='instance', action_name='Remove Instance',
                                          resource_name='Instance', config=_('Instance ID: %s') % obj_id,
                                          status='Success')
    ...
    def handle(self, table, request, obj_ids):
        ...
        action_failure = []
        action_not_allowed = []
        system_disk = request.POST.getlist('system_disk')
        data_disk = request.POST.getlist('data_disk')
        for datum_id, del_system_disk, del_data_disk in zip(obj_ids, system_disk, data_disk):
            ...
            if not table._filter_action(self, request, datum):
                ...
                continue
            try:
                self.action(request, datum_id, del_system_disk, del_data_disk)
...
class RebootInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        ...
        # operation log
        api.logger.Logger(request).create(resource_type='instance', action_name='Reboot Instance',
                                          resource_name='Instance', config=_('Instance ID: %s') % obj_id,
                                          status='Success')
...
class SoftRebootInstance(RebootInstance):
    ...
    def action(self, request, obj_id):
        ...
        # operation log
        config = _('Instance ID: %s') % obj_id

        api.logger.Logger(request).create(resource_type='instance', action_name='Soft Reboot Instance',
...
class LaunchLink(tables.LinkAction):
    ...
    def allowed(self, request, datum):
        ...
        if account_is_frozen:
            ...
            if not (instances_available <= 0 or cores_available <= 0 \
                    or ram_available <= 0):
                self.classes = [c for c in self.classes if c != "disabled"]
...
class CreateSnapshot(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:instances:images:snapshots:create"
    classes = ("ajax-modal",)
    icon = "camera"
...
class ResetPassword(policy.PolicyTargetMixin, tables.LinkAction):
    name = "reset_password"
    verbose_name = _("Reset Password")
    classes = ("btn-reset", "ajax-modal")
    url = "horizon:instances:instances:reset_password"
    policy_rules = (("compute", "compute:reset_password"),)
    ...
    def allowed(self, request, instance):
        ...
        return ((instance.status in ACTIVE_STATES
                 or instance.status == 'SHUTOFF')
                and not is_deleting(instance)
                and (instance.created >= settings.START_TIME_CAN_RESET_PASSWORD))

    def _get_link_url(self, project, step_slug):
        base_url = urlresolvers.reverse(self.url, args=[project.id])
        next_url = self.table.get_full_url()
        params = {"step": step_slug,
                  reset_password.ResetPassword.redirect_param_name: next_url}
        param = urlencode(params)
        return "?".join([base_url, param])

    def get_link_url(self, datum):
        return self._get_link_url(datum, 'reset_password')
...
class RebuildInstance(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:instances:instances:rebuild"
...
class DecryptInstancePassword(tables.LinkAction):
    ...
    url = "horizon:instances:instances:decryptpassword"
    ...
    def single(self, table, request, instance_id):
        try:
            ...
            if fips:
                ...
                # operation log
                config = _('Instance ID: %s Floating IP: %s') %(instance_id, str(fips))
                api.logger.Logger(request).create(resource_type='instance', action_name='Disassociate Float IP',
                                                       resource_name='Instance', config=config,
                                                       status='Success')
            else:
                ...
                # operation log
                api.logger.Logger(request).create(resource_type='instance', action_name='Disassociate Float IP',
                                                       resource_name='Instance', config= _("No floating IPs to disassociate."),
                                                       status='Error')
        except Exception:
            exceptions.handle(request,
            ...
            # operation log
            api.logger.Logger(request).create(resource_type='instance', action_name='Disassociate Float IP',
                                                       resource_name='Instance', config=_("Unable to disassociate floating IP."),
                                                       status='Error')
        return shortcuts.redirect(request.get_full_path())
...
class StartInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        ...
        # operation log
        api.logger.Logger(request).create(resource_type='instance', action_name='Boot Instance',
                                                       resource_name='Instance', config=_('Instance ID: %s') %obj_id,
                                                       status='Success')
...
class StopInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action(self, request, obj_id):
        ...
        # operation log
        api.logger.Logger(request).create(resource_type='instance', action_name='Stop Instance',
                                          resource_name='Instance', config=_('Instance ID: %s') % obj_id,
                                          status='Success')
...
def get_memory_gb(instance):
    if instance.memory_mb >=1024:
        return instance.memory_mb/1024
    else:
        return instance.memory_mb/1024.0
...
def get_size(instance):
    if hasattr(instance, "full_flavor"):
        template_name = 'instances/instances/_instance_flavor.html'
...
def get_cpu_memory(instance):
    cpu = getattr(instance, 'vcpus')
    memory = getattr(instance, 'memory_mb')
    return _('CPU: %dcore Memory: %dMB') % (cpu, memory)


def has_data_volume(instance):
    volume_list = getattr(instance, 'os-extended-volumes:volumes_attached')
...
class InstancesTable(tables.DataTable):
    ...
    config = tables.Column(get_cpu_memory, verbose_name=_("Config"))
    ...
    has_data_volume = tables.Column(has_data_volume,
                                    verbose_name=_("Has data volume"),
                                    hidden=True)
    ...
    class Meta(object):
        ...
        table_actions = launch_actions + (TerminateInstance,
                                          InstancesFilterAction, StartInstance, StopInstance)
        row_actions = (
                       ConfirmResize,
                       RevertResize,
                       EditInstance,
