...
def network_field_data(request, include_empty_option=False):
    ...
    networks = []
    try:
        networks_all = api.neutron.network_list_for_tenant(request, tenant_id)
        networks = [(n.id, n.name_or_id) for n in networks_all]
    ...
    try:
        ...
        ports = api.neutron.port_list_for_tenant(request, tenant_id)
        for network in networks_all:
            if not network.subnets:
                break
            else:
                network_ports = [p for p in ports if p.network_id == network.id]
                if network_ports:
                    router_port = [p for p in network_ports if p.device_owner == 'network:router_interface']
                    if router_port:
                        network_available = True
                        break
        if not network_available:
            return [("", _("(Network not ready)"))]
    except Exception:
        LOG.exception("Failed to retrieve network information")
    return networks
