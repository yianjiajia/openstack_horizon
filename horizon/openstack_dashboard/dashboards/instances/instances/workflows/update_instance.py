...
INDEX_URL = "horizon:instances:instances:index"
ADD_USER_URL = "horizon:instances:instances:create_user"
...
class UpdateInstanceSecurityGroupsAction(workflows.MembershipAction):
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            # operation log
            config = _('Instance ID: %s') %data['instance_id']

            api.logger.Logger(request).create(resource_type='instance', action_name='Update Instance Security Groups',
...
class UpdateInstanceInfoAction(workflows.Action):
    ...
    def handle(self, request, data):
        try:
            ...
            # operation log
            config = _('Instance Name: %s') %data['name']

            api.logger.Logger(request).create(resource_type='instance', action_name='Update Instance Info',
                                                       resource_name='Instance', config=config,
                                                       status='Success')
        except Exception:
