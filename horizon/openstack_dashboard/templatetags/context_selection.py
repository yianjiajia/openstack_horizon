...
from openstack_dashboard.api.billing import getAdvertList
from openstack_dashboard.api.billing import getParentUserById
...
@register.inclusion_tag('context_selection/_project_list.html',
                        takes_context=True)
def show_project_overview(context):
    if 'request' not in context:
        return {}
    request = context['request']
    current_project = filter(lambda project: project.id == request.user.project_id, context['authorized_tenants'])
    context = {'projects': current_project,
               'project_id': request.user.project_id,
               'request': request}
    return context
...
def show_region_list(context):
    ...
    context = {'multi_region':
               ...
               'request': request}
    return context
@register.inclusion_tag('context_selection/_advert.html',
                        takes_context=True)
def show_advert(context):
    if 'request' not in context:
        return {}
    advertList=getAdvertList()
    if advertList['valid_advert_list']:
        for advert in advertList['valid_advert_list']:
            advert['started_at']=advert['started_at'].split(' ')[0]
    context={
...
class getParentUser(template.Node):
    def render(self, context):
        request=context['request']
        parentUser=getParentUserById(request.user.id.encode())
        isSubUser='False'
        if parentUser and parentUser['parentUser']['name'] not in('admin','register'):
            isSubUser='True'
        return isSubUser
    
@register.tag
def isSubUser(parser, token):
    '''判断是否是子账户'''
