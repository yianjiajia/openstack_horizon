...
class BaseUsage(object):
    ...
    def __init__(self, request, project_id=None):
        ...
        self.limits = {}
        self.quotas = {}
        # when no input for date and self is GlobalUsage set this to true, to disable global_usage table loading
    ...
    def _get_neutron_usage(self, limits, resource_name):
        resource_map = {
            ...
            'bandwidth': {
                'api': api.neutron.get_bandwidth, #TODO
                'limit_name': 'totalBandwidthsUsed',
            ...
            'pool': {
                'api': api.lbaas.pool_list,
                'limit_name': 'totalLoadbalancersUsed',
                'message': _('Unable to retrieve Loadbalancers.')
            }
        }
        ...
        try:
            method = resource['api']
            datas = method(self.request)
            if resource_name in('network', 'router', 'pool'):
                datas = [d for d in datas if d.tenant_id == self.request.user.tenant_id]
            current_used = len(datas)
        except Exception:
            current_used = 0
    ...
    def _set_neutron_limit(self, limits, neutron_quotas, resource_name):
        limit_name_map = {
            ...
            'bandwidth': 'maxBandwidths',
            'router': 'maxRouters',
            'network': 'maxNetworks',
            'pool': 'maxLoadbalancers',
        }
    ...
    def get_neutron_limits(self):
        ...
        #bandwidth router network loadbalancer
        # for res in ['bandwidth', 'router', 'network', 'pool']:
        for res in ['router', 'network', 'pool']: #TODO disable bandwidth
            self._get_neutron_usage(self.limits, res)
            self._set_neutron_limit(self.limits, neutron_quotas, res)
