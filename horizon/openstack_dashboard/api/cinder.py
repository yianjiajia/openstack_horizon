...
class VolumeSnapshotStrategy(BaseCinderAPIResourceWrapper):

    _attrs = ['id', 'name', 'description', 'status',
              'created_at', 'volume_id',
              'snapshot_at', 'keep_last_sunday', 'max_keep_number',
              'os-extended-snapshot-attributes:project_id']
...
@memoized
def cinderclient(request, region=None):
    ...
    try:
        ...
        if api_version['version'] == 2:
            try:
                cinder_url = base.url_for(request, 'volumev2', region=region)
...
def volume_list(request, search_opts=None, region=None):
    """To see all volumes in the cloud as an admin you can pass in a special
    search option: {'all_tenants': 1}
    """
    c_client = cinderclient(request, region=region)
    if c_client is None:
        return []
...
def volume_snapshot_list(request, search_opts=None, region=None):
    c_client = cinderclient(request, region=region)
    if c_client is None:
        return []
...
def volume_snapshot_flatten(request, snapshot_id):
    return cinderclient(request).volume_snapshots.flatten(snapshot_id)
...
# add by zhihao.ding 2015/11/24 start
def volume_snapshot_strategy_get(request, strategy_id):
    snapshot = cinderclient(request).volume_snapshot_strategies.get(strategy_id)
    return VolumeSnapshotStrategy(snapshot)

def volume_snapshot_strategy_list(request, search_opts=None):
    c_client = cinderclient(request)
    if c_client is None:
        return []
    return [VolumeSnapshotStrategy(s) for s in c_client.volume_snapshot_strategies.list(
        search_opts=search_opts)]

def volume_snapshot_strategy_create(request, volume_id, name, snapshot_at, keep_last_sunday, max_keep_number, status):
    data = {'name': name,
            'snapshot_at': snapshot_at,
            'keep_last_sunday': keep_last_sunday,
            'max_keep_number': max_keep_number,
            'status': status}
    data = _replace_v2_parameters(data)

    return VolumeSnapshotStrategy(cinderclient(request).volume_snapshot_strategies.create(
        volume_id, **data))

def volume_snapshot_strategy_delete(request, strategy_id):
    return cinderclient(request).volume_snapshot_strategies.delete(strategy_id)

def volume_snapshot_strategy_update(request, strategy_id, name, snapshot_at, keep_last_sunday, max_keep_number, status):
    strategy_data = {'name': name,
                     'snapshot_at': snapshot_at,
                     'keep_last_sunday': keep_last_sunday,
                     'max_keep_number': max_keep_number,
                     'status': status}
    strategy_data = _replace_v2_parameters(strategy_data)
    return cinderclient(request).volume_snapshot_strategies.update(strategy_id,
                                                         **strategy_data)
# add by zhihao.ding 2015/11/24 end
...
def tenant_quota_update(request, tenant_id, region=None, **kwargs):
    return cinderclient(request, region=region).quotas.update(tenant_id, **kwargs)
