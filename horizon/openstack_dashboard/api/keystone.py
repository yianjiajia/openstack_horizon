...
DEFAULT_ROLE = None
DEFAULT_SUPPORT_ROLE = None
DEFAULT_PROJECT_ADMIN_ROLE = None
...
def keystoneclient(request, admin=False):
    ...
    user = request.user
    if admin:
        if (not policy.check((("identity", "admin_required"),), request) and 
            not policy.check((("identity", "project_admin_required"),), request) and
                not policy.check((("identity", "support_required"),), request)):
...
def tenant_list(request, paginate=False, parent_id=None, marker=None, domain=None, user=None,
    ...
    if user == request.user.id:
        ...
        if filters:
            if 'sname' in filters:
                tenants = [t for t in tenants if filters['sname'] in t.name]
            if 'sid' in filters:
                tenants = [t for t in tenants if filters['sid'] in t.id]
...
def user_create(request, name=None, email=None, password=None, project=None,
                region_id=None, enabled=None, domain=None, parent_id=None, **kwargs):
    manager = keystoneclient(request, admin=True).users
    try:
        if VERSIONS.active < 3:
            user = manager.create(name, password, email, project, region_id, enabled)
            return VERSIONS.upgrade_v2_user(user)
        else:
            return manager.create(name, password=password, email=email,
                                  project=project, region_id=region_id, enabled=enabled,
                                  domain=domain, parent_id=parent_id, **kwargs)
...
def get_default_support_role(request):
    """Gets the default role object from Keystone and saves it as a global.

    Since this is configured in settings and should not change from request
    to request. Supports lookup by name or id.
    """
    global DEFAULT_SUPPORT_ROLE
    default_support = getattr(settings, "OPENSTACK_KEYSTONE_DEFAULT_SUPPORT_ROLE", None)
    if default_support and DEFAULT_SUPPORT_ROLE is None:
        try:
            roles = keystoneclient(request, admin=True).roles.list()
        except Exception:
            LOG.warn(traceback.format_exc())
            ...
            exceptions.handle(request)
        for role in roles:
            if role.id == default_support or role.name == default_support:
                DEFAULT_SUPPORT_ROLE = role
                break
    return DEFAULT_SUPPORT_ROLE


def get_default_project_admin_role(request):
    """Gets the default role object from Keystone and saves it as a global.

    Since this is configured in settings and should not change from request
    to request. Supports lookup by name or id.
    """
    global DEFAULT_PROJECT_ADMIN_ROLE
    default_project_admin_support = getattr(settings, "OPENSTACK_KEYSTONE_DEFAULT_PROJECT_ADMIN_ROLE", None)
    if default_project_admin_support and DEFAULT_PROJECT_ADMIN_ROLE is None:
        try:
            roles = keystoneclient(request, admin=True).roles.list()
        except Exception:
            LOG.warn(traceback.format_exc())
            ...
            exceptions.handle(request)
        for role in roles:
            if role.id == default_project_admin_support or role.name == default_project_admin_support:
                DEFAULT_PROJECT_ADMIN_ROLE = role
                break
    return DEFAULT_PROJECT_ADMIN_ROLE
...
def add_region_to_user(request, user_id=None, user_regions=None):
    manager = keystoneclient(request, admin=True).regions
    manager.add_region_to_user(user_id = user_id, user_regions=user_regions)
