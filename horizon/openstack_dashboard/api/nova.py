...
class Server(base.APIResourceWrapper):
    ...
    """
    _attrs = ['addresses', 'attrs', 'id', 'image', 'links', 'networks',
              'metadata', 'name', 'private_ip', 'public_ip', 'status', 'uuid',
              'image_name', 'VirtualInterfaces', 'flavor', 'key_name', 'fault',
              'tenant_id', 'user_id', 'vcpus', 'memory_mb', 'created', 'OS-EXT-STS:power_state',
...
class FloatingIpManager(network_base.FloatingIpManager):
    ...
    def update(self, floating_ip_id, limit_speed):
        raise NotImplemented('for update floatingip limit speed!')
...
def server_delete(request, instance, system_vol_delete_on_termination=0, data_volume_delete_on_termination=0):
    novaclient(request).servers.delete(instance, system_volume=system_vol_delete_on_termination,
                                        data_volume= data_volume_delete_on_termination)

def server_reset_active(request, instance):
    novaclient(request).servers.reset_state(instance, 'active')

def server_get(request, instance_id):
    return Server(novaclient(request).servers.get(instance_id), request)


def server_list(request, search_opts=None, all_tenants=False, region=None):
    page_size = utils.get_page_size(request)
    c = novaclient(request, region=region)
    paginate = False
...
def server_list_admin(request, search_opts=None, all_tenants=False):
    page_size = utils.get_page_size(request)
    ...
    if 'marker' in search_opts:
        marker = search_opts['marker']
    if 'sort_dir' in search_opts:
...
def server_change_user_data(request, instance_id, user_data=None):
    return novaclient(request).servers.change_user_data(instance_id, user_data)
...
def tenant_quota_update(request, tenant_id, region=None, **kwargs):
    novaclient(request, region=region).quotas.update(tenant_id, **kwargs)
...
def interface_attach(request, instance_id, port_id,net_id=None, fixed_ip=None):
    return novaclient(request).servers.interface_attach(instance_id, port_id, net_id=None, fixed_ip=None)

def interface_detach(request, instance_id, port_id):
    novaclient(request).servers.interface_detach(instance_id,port_id)
...
def instance_volumes_detail(request, instance_id):
    volumes = novaclient(request).volumes.get_server_volumes(instance_id)
    return volumes
