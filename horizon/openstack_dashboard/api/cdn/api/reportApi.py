    ...
    def getFlowReport(self, reportForm, domainId = None):
        ...
        try:
            ...
            for i in query_date_period:
                ...
                if ret.status == 200:
                    data = xmlToFlowPointList(ret, reportForm.getReportType())
                    flowreport.append(data)
            return flowreport
        except Exception:
            traceback.print_exc(file=sys.stdout)
            return []
...
class PeriodCheck(object):
    def __init__(self, reportForm):
        self.reportForm = reportForm

    @property
    def dateFrom(self):
        return datetime.datetime.strptime(self.reportForm.getDateFrom(), "%Y-%m-%d %H:%M:%S")

    @property
    def dateTo(self):
        return datetime.datetime.strptime(self.reportForm.getDateTo(), "%Y-%m-%d %H:%M:%S")

    @property
    def date_period(self):
       if (self.dateTo - self.dateFrom).days >= 31:
           return True
       return False

    def date_clean(self):
        if self.date_period:
            middle_date = self.dateFrom+datetime.timedelta(days=15)
            reportForm_1 = ReportForm()
            reportForm_1.reportType = REPORT_TYPE_5_MINUTES
            reportForm_1.dateFrom = self.reportForm.getDateFrom()
            reportForm_1.dateTo = middle_date.strftime("%Y-%m-%d %H:%M:%S")
            reportForm_2 = ReportForm()
            reportForm_2.reportType = REPORT_TYPE_5_MINUTES
            reportForm_2.dateFrom = middle_date.strftime("%Y-%m-%d %H:%M:%S")
            reportForm_2.dateTo = self.reportForm.getDateTo()
            return reportForm_1,reportForm_2
        return (self.reportForm,)
