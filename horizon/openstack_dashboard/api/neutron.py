...
class FloatingIpManager(network_base.FloatingIpManager):
    ...
    def __init__(self, request, region=None):
        self.request = request
        self.client = neutronclient(request, region=region)
    ...
    def list(self, all_tenants=False, **search_opts):
        if not all_tenants:
            ...
            if 'tenant_id' in search_opts:
                search_opts['tenant_id'] = search_opts['tenant_id']
                port_search_opts = {'tenant_id': search_opts['tenant_id']}
            if 'region' in search_opts:
                port_search_opts['region'] = search_opts['region']
    ...
    def allocate(self, pool, limit_speed):
    ...
    def update(self, floating_ip_id, limit_speed):
        body = {'floatingip': {'limit_speed': limit_speed}}
        fip = self.client.update_floatingip(floating_ip_id, body).get('floatingip')
        return FloatingIp(fip)
...
@memoized
def neutronclient(request, region=None):
    insecure = getattr(settings, 'OPENSTACK_SSL_NO_VERIFY', False)
    cacert = getattr(settings, 'OPENSTACK_SSL_CACERT', None)
    c = neutron_client.Client(token=request.user.token.id,
                              auth_url=base.url_for(request, 'identity'),
                              endpoint_url=base.url_for(request, 'network', region=region),
...
def network_list(request, **params):
    ...
    if 'region' in params:
        networks = neutronclient(request, region=params['region']).list_networks(**params).get('networks')
    else:
...
def network_list_admin(request, **params):
    LOG.debug("network_list(): params=%s", params)
    ns = neutronclient(request).list_networks(**params)
...
def subnet_list(request, **params):
    ...
    if 'region' in params:
        subnets = neutronclient(request, region=params['region']).list_subnets(**params).get('subnets')
    else:
...
def port_list(request, **params):
    ...
    if 'region' in params:
        ports = neutronclient(request, region=params['region']).list_ports(**params).get('ports')
    else:
        ports = neutronclient(request).list_ports(**params).get('ports')
    return [Port(p) for p in ports]

def port_list_for_tenant(request,tenant_id,**params):
    LOG.debug("port_list_for_tenant(): tenant_id=%s, params=%s"
              % (tenant_id, params))
    networks=network_list_for_tenant(request, tenant_id,router__external=False)
    print networks
    ports=[]
    if networks:
        for network in networks:
            if network['router__external']==True:
                continue
            ports += neutronclient(request).list_ports(network_id=network.id,**params).get('ports')
    return [Port(p) for p in ports]
...
def router_list(request, **params):
    if 'region' in params:
        routers = neutronclient(request, region=params['region']).list_routers(**params).get('routers')
    else:
        routers = neutronclient(request).list_routers(**params).get('routers')
    return [Router(r) for r in routers]


def router_list_admin(request, **params):
    rs = neutronclient(request).list_routers(**params)
    for r in rs:
...
def tenant_quota_get(request, tenant_id, region=None):
    return base.QuotaSet(neutronclient(request, region=region).show_quota(tenant_id)['quota'])


def tenant_quota_update(request, tenant_id, region=None, **kwargs):
    quotas = {'quota': kwargs}
    return neutronclient(request, region=region).update_quota(tenant_id, quotas)
...
def get_bandwidth(request, **params):
    routers = neutronclient(request).list_routers(**params).get('routers')
    return sum(int(r['bandwidth']) for r in routers)*"M"
