...
class NetworkClient(object):
    def __init__(self, request, region=None):
        ...
        if neutron_enabled:
            self.floating_ips = neutron.FloatingIpManager(request, region=region)
        else:
...
def floating_ip_list(request, region=None, tenant_id=None):
    return NetworkClient(request, region=region).floating_ips.list(all_tenants=True, tenant_id=tenant_id, region=region)


def floating_ip_list_by_ip(request, **kargs):
    return NetworkClient(request).floating_ips.list(all_tenants=True,**kargs)

def tenant_floating_ip_get(request, floating_ip_id):
    return NetworkClient(request).floating_ips.get(floating_ip_id)


def tenant_floating_ip_allocate(request, pool=None, limit_speed=None):
    return NetworkClient(request).floating_ips.allocate(pool, limit_speed)

def tenant_floating_ip_update(request, floating_ip_id, speed_limit):
    return NetworkClient(request).floating_ips.update(floating_ip_id, speed_limit)
