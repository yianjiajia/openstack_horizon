...
from horizon.utils.functions import need_post_check_when_account_is_frozen
...
class ModalFormView(ModalBackdropMixin, ModalFormMixin, views.HorizonFormView):
    ...
    def form_valid(self, form):
        try:
            if not need_post_check_when_account_is_frozen(self):
                handled = form.handle(self.request, form.cleaned_data)
            else:
                handled = None
                form.add_error(None, _('(Account in debt, Please recharge.)'))
        except Exception:
            handled = None
