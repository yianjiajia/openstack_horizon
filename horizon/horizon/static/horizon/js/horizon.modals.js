    ...
    ajaxOpts = {
      ...
      beforeSend: function () {
        ...
        $('.ajax-modal, .dropdown-toggle').attr('disabled', true);
          if($("#images__action_create").length >= 1){
              horizon.modals.modal_spinner(gettext("Uploading,don't leave"));
          }
          else{horizon.modals.modal_spinner(gettext("Working"));}
      },
      complete: function () {
...
//get billing
horizon.modals.get_billing_item = function(){
...
horizon.modals.get_mon_price = function(price){
    var curDate = new Date();
    var curMonth = curDate.getMonth();
    curDate.setMonth(curMonth + 1);
    curDate.setDate(0);
    var days = curDate.getDate();
    return (price*24*days).toFixed(4);
};



horizon.modals.workflow_billing = function(slug){
      var url = window.location.pathname;
      var real_url;
      if (url.split("/")[1] == 'dashboard'){
          real_url = "/dashboard"
    ...
    if (slug == 'flavor_choice'){
        resize_instance(real_url);
    }
    if (slug == 'add_ssl_vpn'){
        ssl_vpn_price(real_url);
    }
};

// resize instance billing
function resize_instance(real_url){
        var instance_id = $("#id_instance_id").val();
        function get_price(data,cpu,mem){
              var cpu_price = data.billing_item.cpu_1_core;
              var mem_price = data.billing_item.memory_1024_M;
              var hour_price = (data.original_price+cpu*cpu_price+(mem*mem_price)/1024).toFixed(4);
              var mon_price = horizon.modals.get_mon_price(hour_price);
              $(".price #price").text(hour_price);
              $(".small-price #mon_price").text(mon_price);

         }
        $.get(real_url+"/instances/api?resize_instance="+instance_id,function(data){
      	  var data_json = eval('(' + data + ')');
          var cpu_value = parseInt($("#id_vcpus").val());
          var mem_value = parseInt($("#id_memory_mb").val());
          get_price(data_json,cpu_value,mem_value);
          $("#total_fee").append("<div class='not_make_sense'>  CPU: "+ "<span class='price'>¥"+ data_json.billing_item.cpu_1_core + "</span>"
              + " /核/时</div><div  class='not_make_sense'>  内存: " +  "<span class='price'>¥" + data_json.billing_item.memory_1024_M + "</span>" + " /G/时</div>");
          $("#id_vcpus").on('change',function(){
              var cpu_value = parseInt($("#id_vcpus").val());
              var mem_value = parseInt($("#id_memory_mb").val());
              get_price(data_json,cpu_value,mem_value);
          });
           $("#id_memory_mb").on('change',function(){
              var cpu_value = parseInt($("#id_vcpus").val());
              var mem_value = parseInt($("#id_memory_mb").val());
              get_price(data_json,cpu_value,mem_value);
          })
...
function ssl_vpn_price(real_url){
    $.get(real_url + "/instances/api?billing_item=billing_item", function (data) {
      data_json = eval('(' + data + ')');
