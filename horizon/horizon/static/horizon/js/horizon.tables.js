...
horizon.datatables.confirm = function (action) {
  var $action = $(action),
    ...
    name_array = [], select_array = [], floating_ip = '',
    closest_table_id, action_string, name_string, system_vol_snap,
    help_text,data_vol_snap,has_data_vol,
  ...
  if ($("#"+closest_table_id+" tr[data-display]").length > 0) {
    ...
    var action_value = $(action).attr("value");
    var patt = new RegExp("instances__terminate");
    if(actions_div.hasClass("table_actions") || actions_div.hasClass("table_actions_menu")) {
      ...
      $("#"+closest_table_id+" tr[data-display]").has(".table-row-multi-select:checkbox:checked").each(function() {
        name_array.push(" \"" + $(this).attr("data-display") + "\"");
          if($("#instances__action_terminate").length && $action.attr('value') == 'instances__terminate') {
              var floating_list = $(this).children().eq(4).find('li');
              if (floating_list.length > 1) {
                  floating_ip = floating_list.last().text();
              }
              has_data_vol =  parseInt($(this).children().eq(9).text());
              var init_data = {
                      'instance_name': $(this).attr("data-display"),
                      'floating_ip': floating_ip
                  };
              if (has_data_vol == 0){
                  init_data['has_not_data_vol'] = true;}
              select_array.push(init_data)
          }
      });
    ...
    else if(actions_div.hasClass('btn-group') && patt.test(action_value)){
        var instance_name = $(action).closest("tr").attr("data-display");
        var floating_list = $(action).closest("tr").children().eq(4).find('li');
        if (floating_list.length > 1){
            floating_ip = floating_list.last().text();}
        has_data_vol =  parseInt($(action).closest("tr").children().eq(9).text());
      }
    else {
  ...
  if($("#instances__action_terminate").length && $action.attr('value') == 'instances__terminate'){
      $("#diy_from_modal_wrapper").children().remove();
      modal = horizon.datatables.del_instance_model(title, select_array, action_string);
  }
  else if(actions_div.hasClass('btn-group') && patt.test(action_value)){
      $("#diy_from_modal_wrapper").children().remove();
      body = {'instance_name':instance_name,
                           'floating_ip': floating_ip};
      if (has_data_vol == 0){
          body['has_not_data_vol'] = true;}
      select_array.push(body);
      modal = horizon.datatables.del_instance_model(title, select_array, action_string);
  }
  else{ modal = horizon.modals.create(title, body, action_string);}
  modal.modal();
...
//del instance modal
horizon.datatables.del_instance_model = function(title, body, confirm, cancel){
  if (!cancel) {
    cancel = gettext("Cancel");
  }
  var template = horizon.templates.compiled_templates["#del_instance_modal_template"],
    params = {
      title: title, body: body, confirm: confirm, cancel: cancel,
      modal_backdrop: horizon.modals.MODAL_BACKDROP
    };
  return $(template.render(params)).appendTo("#diy_from_modal_wrapper");
