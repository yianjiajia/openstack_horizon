...
horizon.Quota = {
  ...
  /*
   * Disable any Memorys for a given image that do not meet
   * its minimum RAM requirements
   */
  disableMemorysForImage: function() {
	image =  horizon.Quota.getSelectedImage();
	if (image === undefined) return;
	
	 if ($('#some_memory_disabled')) {
	      $('#some_memory_disabled').remove();
	 }
	
	var hasDisabled = false;
	var hasEnabled = false;
	memorys = $('#id_memory_mb option');
    memorys.each(function(){
    	if (parseInt($(this).val()) < parseInt(image.min_ram)) {
    ...
    selected = ($('#id_memory_mb option').filter(':selected'))[0];
    if (selected.disabled) {
        memorys.each(function(index, element) {
	        if (!element.disabled) {
	          $('#id_memory_mb').val(element.value);
	          $('#id_memory_mb').change(); // force elements to update
	          return false;
	        }
        });
     }
    
    if ($('#some_memory_disabled').length === 0) {
    	var message = "";
    	if (hasDisabled) {
    		message = (false == hasEnabled) ? horizon.Quota.allFlavorsDisabledMessage : horizon.Quota.disabledFlavorMessage;
    	}
        $('#id_memory_mb').parent().append("<span id='some_memory_disabled'>" + message + '</span>');
    }
  },
  ...
  initWithImages: function(images, disabledMessage, allDisabledMessage) {
    ...
    horizon.Quota.disableMemorysForImage();
  },
  ...
  */
  ...
  updateFlavorUsage: function() {
    ...
    // change by zhihao.ding 2015/6/17 for kill_flavor start
    var old_vcpus = $('#id_old_vcpus').val();
    var old_ram = $('#id_old_memory_mb').val();
    var vcpus = $('#id_vcpus').val();
    var ram = $('#id_memory_mb').val();
    //this.getSelectedFlavor();
    // change by zhihao.ding 2015/6/17 for kill_flavor end
    ...
    $(this.flavor_progress_bars).each(function(index, element) {
      ...
      */
    	  ...
    	  if (old_vcpus) {
    		  update_amount = parseInt(vcpus) < parseInt(old_vcpus) ? 0 : (parseInt(vcpus) - parseInt(old_vcpus));
    	  } else {
    		  update_amount = parseInt(vcpus) * instance_count;
    	  }
      } else if (progress_stat === 'ram') {
    	  if (old_ram) {
    		  update_amount = parseInt(ram) < parseInt(old_vcpus) ? 0 : (parseInt(ram) - parseInt(old_ram));
    	  } else {
    		  update_amount = parseInt(ram) * instance_count;
    	  }
      // change by zhihao.ding 2015/6/17 for kill_flavor end
  ...
  _attachInputHandlers: function() {
    ...
    if(this.is_flavor_quota) {
      ...
      var imageChangeCallback = function(event) {
        ...
        scope.disableMemorysForImage();
      };

      // add by zhihao.ding 2015/7/16 for kill_flavor start
      //$('#id_flavor').on('keyup change', eventCallback);
      $('#id_memory_mb').on('keyup change', eventCallback);
      $('#id_vcpus').on('keyup change', eventCallback);
