...
horizon.membership = {
  ...
  update_membership: function(step_slug) {
      if (step_slug == 'selectimage'){
          $(".btn-group a[href='#add_remove']").hide();
      }
    $(".available_" + step_slug + ", ." + step_slug + "_members").on('click', ".btn-group a[href='#add_remove']", function (evt) {
      ...
      if (available) {
        if (step_slug == 'backendserver'){
          if ($('#id_protocol_port').val().length == 0 || $('#id_weight').val() == 0)
          {
            alert("增加后端服务器时端口和权重必须输入!");
            return;
          }
          data_id_new = data_id + ':Port:' + $('#id_protocol_port').val() + ':Weight:'  + $('#id_weight').val();
          sel = "[data-backendserver-id=" + "'id_backendserver_"  + data_id + "']";
          sel2 = "[value=" + "'" + data_id + "']";
          text = $(sel2).text();
          new_text =text.split(':')[0] + ':' + data_id_new + ':' + text.slice(text.lastIndexOf(':') + 1);
          $(sel2).text(new_text);
          $(sel).children(0).text(new_text);
          $(sel).attr('data-backendserver-id', 'id_backendserver_' + data_id_new);
          $(sel2).attr('value', data_id_new);
          data_id = data_id_new;
        }
        ...
        if (step_slug == 'backendserver'){
          data_id_new = data_id.split(':')[0];
          sel = "[data-backendserver-id=" + "'id_backendserver_"  + data_id + "']";
          sel2 = "[value=" + "'" + data_id + "']";
          text = $(sel2).text();
          arr = text.split(':');
          new_text =arr[0]+ ':' + arr[1] + ':' + arr[6];
          $(sel2).text(new_text);
          $(sel).children(0).text(new_text);
          $(sel).attr('data-backendserver-id', 'id_backendserver_' + data_id_new);
          $(sel2).attr('value', data_id_new);
  ...
  workflow_init: function(modal, step_slug, step_id) {
      ...
      if (step_slug == 'backendserver'){
        $('#id_weight').closest(".form-group").nextAll().hide();
      }
