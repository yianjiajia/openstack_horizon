...
horizon.addInitFunction(horizon.instances.init = function () {
  ...
  $document.on('change', '#id_set_health_check', function (evt) {
      health_check = $('#id_set_health_check');
      if (health_check.val() != "True"){
        elements_list = "#id_type, #id_delay, #id_timeout, #id_max_retries, #id_url_path";
        var elements_list_group = $(elements_list).closest(".form-group");
        elements_list_group.removeClass("required");
        elements_list_group.hide();
      }
      else{
        elements_list = "#id_type, #id_delay, #id_timeout, #id_max_retries, #id_url_path";
        var elements_list_group = $(elements_list).closest(".form-group");
        ...
        elements_list_group.show();
        method = $('#id_type');
        if (method.val() != 'http' && method.val() != 'https'){
          elements_list = "#id_url_path";
          var elements_list_group = $(elements_list).closest(".form-group");
          elements_list_group.hide();
        }
      }
  ...
  $('#id_set_health_check').change();
  horizon.modals.addModalInitFunction(function (modal) {
    $(modal).find("#id_set_health_check").change();
  });
  ...
  function update_device_size() {
    // change by zhihao.ding for kill_flavor start
    //var volume_size = horizon.Quota.getSelectedFlavor().disk;
    var volume_size = 1; // default 1GB
    // change by zhihao.ding for kill_flavor end
